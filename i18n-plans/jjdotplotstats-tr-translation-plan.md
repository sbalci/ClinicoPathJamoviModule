# Internationalization (i18n) Translation Plan: jjdotplotstats → Turkish (TR)

## 0) Argument Normalization

**SANITIZED_FN**: `jjdotplotstats`
...
- ✅ All user-visible strings in `R/jjdotplotstats.b.R` and YAML definitions are wrapped in `.(...)` or declared in schemas.
...
| :--- | :--- | :--- |
| `Horizontal Box-Violin Comparison` | `Yatay Kutu-Keman Karşılaştırması` | Başlık |
...
| `Effect Size Measure` | `Effect Size Measure` | Seçenek başlığı |
| `Central tendency lines` | `Central tendency lines` | Seçenek başlığı |
| `Central Tendency Measure` | `Central Tendency Measure` | Seçenek başlığı |
| `Plot Title` | `Plot Title` | Seçenek başlığı |
| `X-axis Label (Values)` | `X-axis Label (Values)` | Seçenek başlığı |
...
| `Reference Line Value` | `Reference Line Value` | Seçenek başlığı |
| `Bayes factor interpretation` | `Bayes factor interpretation` | Seçenek başlığı |
| `Confidence Level` | `Confidence Level` | Seçenek başlığı |
| `Statistical Precision (Decimal Places)` | `Statistical Precision (Decimal Places)` | Seçenek başlığı |
| `Reference value line` | `Reference value line` | Seçenek başlığı |
| `Central Tendency Display` | `Central Tendency Display` | Seçenek başlığı |
| `Central Tendency Precision (no longer used)` | `Central Tendency Precision (no longer used)` | Seçenek başlığı |
...
- [x] User-facing strings in `R/jjdotplotstats.b.R` properly wrapped in `.(...)`.
