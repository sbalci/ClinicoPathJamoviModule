...
**Options covered:** `outcome`, `positiveClass`, `predictors`,
`analysisType` (single), `direction`, `youdenOptimization`,
`customCutoffs`, `aucTable`, `optimalCutoffs`, `diagnosticMetrics`,
`cutoffTable`, `clinicalMetrics`, `prevalence`, `useObservedPrevalence`
...
**Options covered:** `analysisType` (comparative, comprehensive),
`pairwiseComparisons`, `comparisonMethod`
(delong/bootstrap/venkatraman), `showMetricsDiff`,
`statisticalComparison`, `comprehensive_output`,
...
**Options covered:** `confidenceLevel`, `useBootstrap`,
`bootstrapSamples`, `bootstrapMethod` (bca/percentile/basic),
`bootstrapCutoffCI`, `bootstrapPartialAUC`, `stratifiedBootstrap`
...
**Options covered:** `rocCurve`, `showCutoffPoints`,
`showConfidenceBands`, `plotTheme` (classic/modern/clinical),
...
**Options covered:** Youden Index plot, Cutoff Analysis plot, Clinical
Decision plot (all triggered by corresponding table options)
...
**Options covered:** `smoothMethod` (none/binormal/kernel),
`partialAuc`, `partialAucType` (specificity/sensitivity),
`partialRange`, `crocAnalysis`, `crocAlpha`, `convexHull`,
`tiedScoreHandling` (average/upper/lower)
...
**Options covered:** `detectImbalance`, `imbalanceThreshold`,
`showImbalanceWarning`, `recommendPRC`
...
**Options covered:** `clinicalContext`
(screening/diagnosis/prognosis/monitoring/general), `clinicalPresets`
(custom/biomarker_screening/diagnostic_validation/confirmatory_testing/research_comprehensive)
...
**Options covered:** `calibrationAnalysis`, `calibrationPlot`,
`hosmerLemeshow`, `hlGroups`, `brierScore`, `calibrationMetrics`
...
**Options covered:** `multiClassROC`, `multiClassStrategy` (ovr/ovo),
`multiClassAveraging` (macro/weighted)
...
**Options covered:** `clinicalImpact`, `nntCalculation`,
`clinicalUtilityCurve`, `decisionImpactTable`
...
**Options covered:** small sample handling, low prevalence warnings,
tied score edge cases

------------------------------------------------------------------------

## 14. NOT YET IMPLEMENTED (Planned Features)

These options exist in YAML but are **not yet implemented** in the
backend. Verify they show appropriate “not implemented” notices or are
silently skipped:
...
`predictors` — all tests

### Analysis Type & Direction

`analysisType` (single/comparative/comprehensive) — \#1, \#4, \#6

`direction` (auto/higher/lower) — \#3

### Cutoff Options

`youdenOptimization` — \#1, \#13

`customCutoffs` — \#3

`sensitivityThreshold` — \#22, \#23

`specificityThreshold` — \#22, \#23
...
`bootstrapSamples` — \#7, \#37

`bootstrapMethod` (bca/percentile/basic) — \#7, \#8

`bootstrapCutoffCI` — \#9

`bootstrapPartialAUC` — \#9

`stratifiedBootstrap` — \#8

### Comparison

`pairwiseComparisons` — \#4

`comparisonMethod` (delong/bootstrap/venkatraman) — \#4, \#5
...
`clinicalMetrics` — \#2, \#15

### Advanced ROC

`smoothMethod` (none/binormal/kernel) — \#16

`partialAuc`, `partialAucType`, `partialRange` — \#17, \#18

`crocAnalysis`, `crocAlpha` — \#19

`convexHull` — \#20

`tiedScoreHandling` (average/upper/lower) — \#21

### Class Imbalance

`detectImbalance` — \#24

`imbalanceThreshold` — \#24, \#25

`showImbalanceWarning` — \#24

`recommendPRC` — \#24

### Clinical Context

`prevalence`, `useObservedPrevalence` — \#2

`clinicalContext` (screening/diagnosis/prognosis/monitoring/general) —
\#26-#30
...
`comprehensive_output` — \#6

`clinical_interpretation` — \#6

### Calibration

`calibrationAnalysis` — \#31

`calibrationPlot` — \#31

`hosmerLemeshow`, `hlGroups` — \#32

`brierScore` — \#31

`calibrationMetrics` — \#31

### Multi-Class

`multiClassROC` — \#33

`multiClassStrategy` (ovr/ovo) — \#33, \#34

`multiClassAveraging` (macro/weighted) — \#33, \#34
...
`clinicalUtilityCurve` — \#36

`decisionImpactTable` — \#35

### Plot Settings

`plotTheme` (classic/modern/clinical) — \#10, \#11

`plotWidth`, `plotHeight` — \#11

`showCutoffPoints` — \#10

`showConfidenceBands` — \#10

### Comparison Display

`showMetricsDiff` — \#5

`statisticalComparison` — \#5

### Not Yet Implemented (Verify No Errors)

`splineCalibration`, `splineKnots` — \#40

`eoRatio`, `namDagostino`, `greenwoodNam` — \#41

`calibrationBelt`, `calibrationDensity` — \#42

`harrellCIndex`, `unoCStatistic` — \#43

`incidentDynamic`, `cumulativeDynamic`, `competingRisksConcordance` —
\#44

`internalValidation`, `validationMethod`, `optimismCorrection` — \#45

`externalValidation`, `decisionImpactCurves`, `netBenefitRegression` —
\#46

`modelUpdating`, `transportability` — \#47
