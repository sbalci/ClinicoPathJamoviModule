...
|---------|---|-----------|--------|-------------|
| `grouplasso_biomarker` | 200 | 15 (mixed numeric + factor) | 91 | Breast cancer biomarker panel with 5 clinical groups |
| `grouplasso_genomic` | 120 | 30 (all continuous) | 59 | Gene expression data with 6 pathway groups |
| `grouplasso_small` | 60 | 8 (mixed) | 31 | Small clinical cohort for edge-case testing |
...
biomarker <- grouplasso_biomarker
str(biomarker)
...
Look for: The suitability report flags the small sample size and the low EPV. With only 31 events, cross-validation keeps no group at lambda.min, so all coefficients are zero and the output shows a "No Variables Selected" warning.
...
Look for: Each factor variable (grade, lvi, her2, chemo, radiation, hormonal) forms its own group with all its dummy variables included together. With these six factors alone, no group is selected at lambda.min, so the output shows a "No Variables Selected" warning.
