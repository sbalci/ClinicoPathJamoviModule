...
Compare the two: lower alpha tends to select/exclude entire groups, higher alpha allows individual variable selection within retained groups. Here every continuous predictor forms its own group, so both fits keep all 8 variables; the difference shows only when groups contain several variables (factors or custom groups).
...
BIC typically selects fewer variables than CV deviance or AIC due to its stronger penalty for model complexity. In this example AIC keeps 4 of the 8 variables and BIC keeps none, so the BIC output shows a "No Variables Selected" warning.
...
genepanel <- sparsegrouplasso_genepanel
...
sparsegrouplasso(
  ...
  censorLevel = "Stable",
  pred_vars = setdiff(names(genepanel), c("time", "status")),
  pathway_info = NULL,
...
With 40 predictors and 100 observations, this is a p > n/3 scenario where regularization is essential. With these settings, correlation-based grouping at r = 0.5 merges only a few genes (37 groups) and cross-validation keeps no variable, so the output shows a "No Variables Selected" warning.
...
The suitability report flags the low EPV (19 events for 4 predictors), and a "Low Event Count" warning is shown.
