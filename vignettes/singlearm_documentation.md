# Single Arm Survival - Feature Mapping Specification

## Feature-to-Code Mapping

- **Analysis Function**: `singlearm`
- **Module**: `SurvivalT`
...
| `baseline_hazard` | UI Control `baseline_hazard` | `self$options$baseline_hazard` | Output item / Table |
| `hazard_smoothing` | UI Control `hazard_smoothing` | `self$options$hazard_smoothing` | Output item / Table |
...
| `advancedDiagnostics` | UI Control `advancedDiagnostics` | `self$options$advancedDiagnostics` | Output item / Table |

## Verification Checklist

- [x] All options defined in `.a.yaml` have matching UI bindings in `.u.yaml`.
- [x] Backend `R/singlearm.b.R` references all declared options safely.
