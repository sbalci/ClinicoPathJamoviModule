# Testing Checklist: Lowest Expected Value for a fixed sample size (`kappaSizeFixedN`)
...
Run the dedicated test suite for `kappaSizeFixedN`:

```r
testthat::test_file("tests/testthat/test-kappaSizeFixedN.R")
```
