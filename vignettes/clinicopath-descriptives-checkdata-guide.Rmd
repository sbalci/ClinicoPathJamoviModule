...
- **MCAR explanation option**: states why the mechanism cannot be tested from one variable and points to `naniar::mcar_test()` on the full dataset
...
- **Unit-aware validation** for:
  ...
  - Weight (kg)
  - Height (cm or metres, chosen from the value range)
  - Lab values (hemoglobin, creatinine with SI vs traditional units)
- Units are inferred from the value range; there is no unit-system option
...
- Units are inferred from the value range (kg; cm or m; g/dL or g/L; mg/dL or µmol/L). Imperial units are not supported.

#### Missing Data Analysis
- **Explain MCAR testability** (default: false)
  - Adds a statement that Little's MCAR test is multivariate and cannot be computed from a single variable
  - No test is performed; run `naniar::mcar_test()` on the full dataset for a formal test
...
**Weight**:
- Checked in kg (plausible range 2-200)
- Outside range → "verify units or data entry"
- Shows detected unit: "(assumed kg)"

**Height**:
- cm (50-250) when the maximum exceeds 10, otherwise metres (0.5-2.5)
...
- **Mixed units in dataset**: unit inference from the value range may fail
- **Population differences**: Different normal ranges by ethnicity

**Action**:
- Review flagged values in clinical context
- Check the units of flagged values (cm vs m, mg/dL vs µmol/L)
...
2. **Configure Appropriately**:
   - Know the units your columns are recorded in; the checks infer them from the value range
...
4. **Clinical Checks**:
   ...
   - ✅ **Do**: Confirm the units of flagged values before acting
...
- Run `naniar::mcar_test()` on the full dataset for a formal assessment
...
**Mitigation**:
- Check the units of flagged values when the inferred unit looks wrong
...
3. **Option 3**: Confirm the units of the flagged values
...
### The MCAR explanation option

Little's MCAR test is multivariate: it compares observed means across missingness patterns using the other variables. It is undefined for a single variable, so this analysis does not run it. Enabling **Explain MCAR testability** adds that statement to the Quality Assessment Summary (and to Data Patterns when that table is shown).

**Action**: to test MCAR formally, run `naniar::mcar_test()` on the full dataset and combine the result with the heuristic runs and dropout findings reported here.
...
**Solution**:
1. Check the units of the column:
   - Units are inferred from the value range and mixed units defeat that
   - Convert the column to one unit (cm, kg, one lab unit) and re-run
...
- Record each column in a single unit
...
- Confirm the units of flagged values
...
- Not used here: `naniar::mcar_test()` is recommended on the full dataset for a formal MCAR test
...
- ✅ MCAR explanation option stating why the test cannot run on one variable
...
**Clinical Validation**:
- ✅ Units inferred from the value range (weight, height, lab values); imperial units not supported
