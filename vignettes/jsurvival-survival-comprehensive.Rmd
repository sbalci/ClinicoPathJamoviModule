...
result_wlr <- survival(
  ...
  survivalTestType = "fh_rho0_5"
)
```

When enabled, the function runs the three tests of the Fleming-Harrington family that
`survival::survdiff(rho = )` provides:

| Test | rho | Emphasizes |
|------|-----|------------|
| Log-Rank (standard) | 0 | All time points equally |
| Fleming-Harrington | 0.5 | Moderate early weighting |
| Peto-Peto | 1 | Early differences |
...
`export_survival_data` is an **Output** option: it writes the estimates back into the jamovi
spreadsheet as a new column. jamovi's compiler does not turn Output options into arguments of the
generated R function, so this one is available from the jamovi interface only — passing
`export_survival_data = TRUE` to `survival()` from R raises `unused argument`. The same applies to
this analysis's other two Output options, `calculatedtime` and `outcomeredefined`.

In jamovi, tick **Export Estimated Survival** in the options panel. From R, take the estimates off
the results object instead:
