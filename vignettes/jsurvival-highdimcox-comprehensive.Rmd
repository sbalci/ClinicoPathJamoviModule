...
# Variable arguments are read as column names: a bare `gene_vars` would be
# looked up as a column, so `!!` passes the names the vector holds.
highdimcox(
  data = data_genomic,
...
With fewer than 30 observations the analysis stops with a validation message.
...
When no row matches the event level, the analysis stops with a validation message.
