# Testing Checklist: Sequential Testing Analysis (`sequentialtests`)
...
Run the dedicated test suite for `sequentialtests`:

```r
testthat::test_file("tests/testthat/test-sequentialtests.R")
```
