...
The examples in this section are shown but not run: in the current version a
non-empty `markers` list is rejected with "Argument 'markers' requires a numeric
variable", even for numeric columns, so ROC comparison is unavailable until that
defect is fixed.

### 7a. DeLong Test (Default)

```{r roc-comparison-delong, eval=FALSE}
# Not run: comparison markers currently fail the option check (package bug in timeroc).
timeroc(
  data = timeroc_test,
  outcome = "Recurrence",
  outcomeLevel = "1",
...
```{r roc-comparison-bootstrap, eval=FALSE}
# Not run: comparison markers currently fail the option check (package bug in timeroc).
timeroc(
  data = timeroc_test,
  outcome = "Recurrence",
...
```{r roc-comparison-venkatraman, eval=FALSE}
# Not run: comparison markers currently fail the option check (package bug in timeroc).
timeroc(
  data = timeroc_test,
  outcome = "Recurrence",
