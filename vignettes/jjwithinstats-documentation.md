# Box-Violin Plots to Compare Within Groups - Developer Documentation

## 1. Overview

- **Function**: `jjwithinstats`
- **Title**: Box-Violin Plots to Compare Within Groups
- **Module**: `JJStatsPlotT`
- **Files**:
  - `jamovi/jjwithinstats.u.yaml` - User Interface Definition
  - `jamovi/jjwithinstats.a.yaml` - Options & Schema Definition
  - `jamovi/jjwithinstats.r.yaml` - Results Layout & Tables
  - `R/jjwithinstats.b.R` - Backend Implementation
- **Summary**: Wrapper Function for ggstatsplot::ggwithinstats and ggstatsplot::grouped_ggwithinstats to generate violin plots for repeated measurements and within-subjects analysis with statistical annotations and significance testing.

## 1a. Changelog

- **Date**: 2026-08-29
- **Summary**: Comprehensive documentation suite created & verified against active schemas and backend implementation.

## 2. Options Reference (`.a.yaml`)

| Option | Type | Default | Description |
| :--- | :--- | :--- | :--- |
| `data` | `Data` | `NULL` |  |
| `dep1` | `Variable` | `NULL` | First Measurement |
| `dep2` | `Variable` | `NULL` | Second Measurement |
| `dep3` | `Variable` | `NULL` | Third Measurement (Optional) |
| `dep4` | `Variable` | `NULL` | Fourth Measurement (Optional) |
| `pointpath` | `Bool` | `FALSE` | Point path |
| `centralitypath` | `Bool` | `FALSE` | Centrality path |
| `centralityplotting` | `Bool` | `FALSE` | Average values |
| `centralitytype` | `List` | `parametric` | Centrality Type |
| `clinicalpreset` | `List` | `custom` | Clinical Analysis Preset |
| `typestatistics` | `List` | `parametric` | Statistical Test Type |
| `pairwisecomparisons` | `Bool` | `FALSE` | Compare each time point pair |
| `pairwisedisplay` | `List` | `significant` | Pairwise Display |
| `padjustmethod` | `List` | `holm` | Adjustment Method |
| `effsizetype` | `List` | `biased` | Effect Size Needed for Parametric Tests |
| `violin` | `Bool` | `TRUE` | Violin plot |
| `boxplot` | `Bool` | `TRUE` | Box plot |
| `point` | `Bool` | `TRUE` | Points |
| `mytitle` | `String` | `Within Group Comparison` | Title |
| `xtitle` | `String` | `` | X-Title |
| `ytitle` | `String` | `` | Y-Title |
| `originaltheme` | `Bool` | `FALSE` | Add GGStatsPlot layer |
| `resultssubtitle` | `Bool` | `FALSE` | Statistical results |
| `bfmessage` | `Bool` | `FALSE` | Bayes factor message |
| `conflevel` | `Number` | `0.95` | Confidence Level |
| `k` | `Integer` | `2` | Decimal Places |
| `plotwidth` | `Integer` | `650` | Plot Width |
| `plotheight` | `Integer` | `450` | Plot Height |
| `addGGPubrPlot` | `Bool` | `FALSE` | Add ggpubr plot variant |
| `ggpubrPlotType` | `List` | `boxplot` | ggpubr Plot Type |
| `ggpubrPalette` | `List` | `jco` | ggpubr Color Palette |
| `ggpubrAddStats` | `Bool` | `FALSE` | Add statistical comparisons (ggpubr) |
| `ggpubrShowLines` | `Bool` | `FALSE` | Connecting lines (paired plot) |
| `ggpubrAddPoints` | `Bool` | `FALSE` | Add individual points |
| `showExplanations` | `Bool` | `TRUE` | Explanations |

## 3. Results Definition (`.r.yaml`)

| Output ID | Type | Title | Description |
| :--- | :--- | :--- | :--- |
| `todo` | `Html` | `Analysis Guide` |  |
| `warnings` | `Html` | `Messages` |  |
| `interpretation` | `Html` | `Clinical Interpretation` |  |
| `explanations` | `Html` | `Explanations` |  |
| `plot` | `Image` | `Violin Plots` |  |
| `summary` | `Html` | `Analysis Summary` |  |
| `ggpubrPlot` | `Image` | `Publication-Ready Within-Subjects Plot (ggpubr)` |  |

## 4. Architecture & Data Flow Diagram

```mermaid
flowchart TD
  subgraph UI[jamovi UI / .u.yaml]
    U1[User Input & Variables]
    U2[Analysis Settings & Controls]
  end

  subgraph Opts[Options Schema / .a.yaml]
    O1[Options Parsing & Types]
    O2[Default Value Validation]
  end

  subgraph Backend[Backend Logic / R/jjwithinstats.b.R]
    B1[Input Validation & Data Sanitization]
    B2[Statistical Computation Engine]
    B3[Result Objects Formatting]
  end

  subgraph Res[Results Schema / .r.yaml]
    R1[Summary & Statistics Tables]
    R2[Visual Plots & Graphics]
    R3[Clinical Interpretation & Notices]
  end

  U1 --> O1
  U2 --> O2
  O1 --> B1
  O2 --> B1
  B1 --> B2
  B2 --> B3
  B3 --> R1
  B3 --> R2
  B3 --> R3
```

## 5. Execution Sequence

```mermaid
sequenceDiagram
  autonumber
  actor User as Clinician / Analyst
  participant UI as jamovi Interface
  participant Backend as R Backend (jjwithinstatsClass)
  participant Engine as Statistical Packages
  participant Results as Results View

  User->>UI: Selects variables and options
  UI->>Backend: Dispatches .run() with options payload
  Backend->>Backend: Validates observations & factor levels
  Backend->>Engine: Computes statistical models / visual layers
  Engine-->>Backend: Returns model estimates & graphics
  Backend->>Results: Populates tables, charts, and notices
  Results-->>User: Displays formatted tables & interactive plots
```

## 6. Change Impact & Safety Guidelines

- **Data Filtering**: Ensure observations with missing values are handled gracefully according to analysis options.
- **Formula Conflicts**: Use isolated environment calls or base formula methods when interacting with `ggstatsplot` or formula parsers.
- **Safe Deparsing**: Use `deparse(val)` in syntax generation (`asSource()`) to escape column names with spaces or special symbols.

