...
```{r}
clean_vartree_html <- function(vartree_output, filepath = "vartree_output_clean.html") {
  raw <- vartree_output$asString()

  # Remove console clutter and any outer <div id="myDIV"> ... </div>
  lines <- unlist(strsplit(raw, "\n"))

  # Filter out unwanted lines
  cleaned <- lines[!grepl("^\\s*(VARIABLE TREE|character\\(0\\)|<div id=\\\"myDIV\\\"|</div>|#myDIV)", lines)]

  # Collapse cleaned lines back to a single string
  cleaned_svg <- paste(cleaned, collapse = "\n")

  # Wrap cleaned SVG in minimal valid HTML
  html_output <- paste0(
    "<!DOCTYPE html>\n<html>\n<head>\n",
    "<meta charset='UTF-8'>\n",
    "<style>#myDIV {width: 1000px; height: 850px; overflow: auto;}</style>\n",
  ...
  writeLines(html_output, filepath)
  browseURL(filepath)
}

# Run the function
clean_vartree_html(result1, "./vignettes/output/vartree_output_clean.html")



# Wrap the raw SVG+HTML fragment into a full HTML document
html_corrected <- paste0(
  "<!DOCTYPE html>\n<html>\n<head>\n",
  "<meta charset='UTF-8'>\n",
  "<style>#myDIV {width: 1000px; height: 850px; overflow: auto;}</style>\n",
  "</head>\n<body>\n",
...
writeLines(html_corrected, "./vignettes/output/vartree_output.html")



result3 <- DiagrammeRsvg::export_svg(gv = result2)


# Export SVG to file or view in RStudio
result1 <- result1$asString()

# result1 <- base::sub('width=\"[[:digit:]pt\"]+',
#                                   ifelse(horizontal == TRUE, 'width=400pt ', 'width=1000pt '),
#                                   result1)

result1 <- paste0('<html><head><style>
                               #myDIV {width: 610px; height: 850px; overflow: auto;}
                               </style></head><body><div id="myDIV">',
                               result1,
                               '</div></script></body></html>')

writeLines(result1, "./vignettes/output/vartree_output.html")
...
```{r}
svg_text <- DiagrammeRsvg::export_svg(result1)
svg_parsed <- xml2::read_xml(svg_text)

# View in Viewer pane (if supported) or save
html_output <- paste0('<html><head><style>
                      #myDIV {width: 1000px; height: 850px; overflow: auto;}
                      </style></head><body><div id="myDIV">',
                      svg_text,
                      '</div></body></html>')

# Save to HTML file to view in browser
writeLines(html_output, "vartree_output.html")
browseURL("vartree_output.html")




result1 <- capture.output(cat(result1))

results1 <- DiagrammeRsvg::export_svg(gv = result1)
