...
- Use `R >= 4.5.0`
- Install jamovi from https://www.jamovi.org/download.html
- Install `jmvtools`. It is **not on CRAN** - it must be installed from the jamovi
  repository, along with its `node` dependency:
...
`jmvtools` is not on CRAN, so install it (and its `node` dependency) from the
jamovi repository:
  ...
  - type: Label
    ...
    children:
      ...
      - type: TextBox
        name: pprob
        format: number
        enable: (pp)               # Gated on its own checkbox only
```

> This panel used to disable `pp` whenever `ci` was checked, and gate `pprob` on
> `(!ci && pp)`. That mutual exclusion was removed: Clopper-Pearson intervals for
> sensitivity and specificity are prevalence-independent, so they stay valid when a
> population prior is supplied. For a genuine multi-condition example see
> "Conditional Enabling" below.
