...
**Date:** 2026-01-31 **Status:** Files are CORRECT - Issue is jamovi
cache
...
The error occurs because **jamovi is using a cached version** of the
module that was loaded before the fix was applied.
...
3.  **Development Module Cache** - When running from development
    directory
...
✅ Source files (.r.yaml, .b.R) are updated - ✅ Compiled files (.h.R,
.src.js) are regenerated - ❌ But jamovi has ALREADY loaded the old
module into memory - ❌ jamovi won’t reload until you force it to
...
**Why this works:** Replaces the installed cached version with the fixed
version.
...
The summary table should show 5 rows: 1. Hazard Consistency 2. Hazard
Discrimination 3. Sample Balance 4. Outcome Prediction 5. Overall Rank
...
Check the version number and install date. If the date is BEFORE today
(2026-01-31), the module wasn’t reinstalled.
...
Make sure you don’t have multiple jamovi windows open with different
module versions:
...
This shows where jamovi is loading the module from. If it’s NOT pointing
to your development directory, you need to reinstall.
...
This happens because jamovi’s module loading system caches modules
aggressively for performance. The cache doesn’t automatically invalidate
...
**Generated:** 2026-01-31 19:30 **Status:** Ready for user action
