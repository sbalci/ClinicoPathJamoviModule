...
methods, outlier detection, diagnostic plots, and bootstrap confidence
intervals. Includes Spearman, Kendall, Percentage Bend, Biweight
Midcorrelation, Minimum Volume Ellipsoid (MVE), and Minimum Covariance
Determinant (MCD) correlation methods.
...
- dep:
  ...
  List of continuous variables for robust correlation analysis. All
  variables must be numeric with sufficient variation.
...
- outlier_threshold:
  ...
  Threshold for outlier detection (in standard deviations or
  equivalent).
...
- low_color:
  ...
  Color for low (negative) correlation values (when using custom
  colors).
...
- high_color:
  ...
  Color for high (positive) correlation values (when using custom
  colors).
