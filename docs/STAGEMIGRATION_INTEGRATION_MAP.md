...
**Purpose:** Complete mapping of dataflow across all stagemigration
files **Date:** 2026-01-26 **Status:** Integration Guide for .a.yaml →
.b.R → utils → .r.yaml
...
|----|----|----|----|----|
| `data` | VariableSupplier | `self$data` | [`stagemigration_validateData()`](https://www.serdarbalci.com/ClinicoPathJamoviModule/reference/stagemigration_validateData.md) | N/A (internal) |
...
**Integration Status:** ✅ All core variables must be validated before
any analysis
...
**Add to .u.yaml:** Already present in reorganized version at top of
Section 2
...
**No .r.yaml changes needed** - results visibility controlled by
existing options
...
**Integration Decision:** - Keep both but make `analysisType` hidden
(backward compatibility) - Primary UI control: `complexityMode` (simpler
names: quick/standard/comprehensive/custom) - `clinicalPreset` sets
initial defaults for complex options - `analysisType` = `complexityMode`
...
| **Utility function** | ✅ [`stagemigration_calculateConcordance()`](https://www.serdarbalci.com/ClinicoPathJamoviModule/reference/stagemigration_calculateConcordance.md) | **stagemigration-discrimination.R** |
...
| **Utility function** | ✅ [`stagemigration_competingRisksAnalysis()`](https://www.serdarbalci.com/ClinicoPathJamoviModule/reference/stagemigration_competingRisksAnalysis.md) | **stagemigration-competing-risks.R** |
...
| **Utility function** | ✅ [`stagemigration_calculateRMST()`](https://www.serdarbalci.com/ClinicoPathJamoviModule/reference/stagemigration_calculateRMST.md) | **stagemigration-competing-risks.R** |
| **.r.yaml output** | ⚠️ **MISSING** - Need to add | **ADD NEW TABLES:** |
...
| **.b.R usage** | ❌ Not implemented | **IMPLEMENT:** |
| **Utility function** | ✅ [`stagemigration_cutpointAnalysis()`](https://www.serdarbalci.com/ClinicoPathJamoviModule/reference/stagemigration_cutpointAnalysis.md) | **stagemigration-competing-risks.R** |
| **.r.yaml output** | ⚠️ **MISSING** - Need to add | **ADD NEW TABLES:** |
...
**Questions?** This integration map should guide the complete
refactoring.

**Key Files:** - `.a.yaml` - Options definition - `.u.yaml` - UI
controls (use reorganized version) - `.b.R` - Backend logic (main
integration work) - `.r.yaml` - Result outputs (add missing items) -
`*-utils.R`, `*-validation.R`, `*-discrimination.R`,
`*-competing-risks.R` - Utility modules

**Next Step:** Create integration script that systematically updates
.b.R
