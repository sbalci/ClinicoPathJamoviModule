  ...
  R6::R6Class(
    ...
    private = list(
      ...
      .fitAalenModel = function(data_prep) {
        ...
        # Backtick-quote names so non-syntactic identifiers (e.g., "Tumor Size") survive formula parsing.
        covar_terms <- vapply(covar_names, jmvcore::composeTerm, character(1))
        const_terms <- if (!is.null(const_names) && length(const_names) > 0)
            vapply(const_names, jmvcore::composeTerm, character(1)) else character(0)

        if (model_type == "additive") {
          # Pure additive model - all effects time-varying
          formula_str <- paste0("survival ~ ", paste(covar_terms, collapse = " + "))
          ...
          varying_terms <- vapply(varying_names, jmvcore::composeTerm, character(1))

          if (length(const_names) > 0 && length(varying_names) > 0) {
            formula_str <- paste0("survival ~ const(", paste(const_terms, collapse = ", "), ") + ",
                                paste(varying_terms, collapse = " + "))
      ...
      .generateExplanations = function() {
        explanations <- paste0(
          ...
          "<p><strong>\u{03BB}(t|X) = \u{03BB}\u{2080}(t) + \u{03B2}\u{2081}(t)X\u{2081} + \u{03B2}\u{2082}(t)X\u{2082} + ... + \u{03B2}\u{209A}(t)X\u{209A}</strong></p>",
          "<p>where \u{03BB}\u{2080}(t) is the baseline hazard and \u{03B2}(t) are time-varying regression coefficients.</p>",
          ...
          "<li><strong>Cumulative Regression Functions:</strong> \u{03B2}\u{0302}(t) shows how effects accumulate</li>",
