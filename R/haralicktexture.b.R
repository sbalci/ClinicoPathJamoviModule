...
haralicktextureClass <- if (requireNamespace("jmvcore", quietly = TRUE)) {
    R6::R6Class(
        ...
        private = list(
            ...
            .analyzeGroupComparisons = function(texture_matrix, texture_cols) {
                ...
                if (!group_col %in% names(texture_matrix)) {
                    return()
                }
                ...
                if (length(group_levels) != 2) {
                    return()
                } # Only handle two groups for now
                ...
                for (feature in texture_cols) {
                    if (feature %in% names(spatial_data)) {
                        ...
                        spatial_interpretation <- if (overall_spatial_score > 0.7) {
                            paste0(
                                "Strong spatial heterogeneity (Score: ", round(overall_spatial_score, 3), ") - ",
                                "Significant spatial clustering and regional variation detected"
                            )
                        } else if (overall_spatial_score > 0.5) {
                            paste0(
                                "Moderate spatial heterogeneity (Score: ", round(overall_spatial_score, 3), ") - ",
                                "Some spatial structure present"
                            )
                        } else if (overall_spatial_score > 0.3) {
                            paste0(
                                "Weak spatial heterogeneity (Score: ", round(overall_spatial_score, 3), ") - ",
                                "Limited spatial organization"
                            )
                        } else {
                            paste0(
                                "Low spatial heterogeneity (Score: ", round(overall_spatial_score, 3), ") - ",
            ...
            .analyzePrognosticValue = function(texture_data, texture_cols) {
                ...
                if (!outcome_var %in% names(texture_data)) {
                    return()
                }
                ...
                if (biomarker_context == "ki67") {
                    biomarker_interpretation <- paste0(
                        ...
                        if (has_entropy && entropy_mean > 2.5) {
                            paste0(
                                "<p> <strong>High Ki67 entropy</strong> (mean = ", round(entropy_mean, 2),
                            ...
                            paste0(
                                "<p> <strong>Moderate Ki67 entropy</strong> (mean = ", round(entropy_mean, 2),
                            ...
                            paste0(
                                "<p> <strong>Low Ki67 entropy</strong> (mean = ", round(entropy_mean, 2),
                    ...
                    biomarker_interpretation <- paste0(
                        ...
                        if (has_entropy && entropy_mean > 2.0) {
                            paste0(
                                "<p> <strong>High HER2 spatial heterogeneity</strong> (entropy = ", round(entropy_mean, 2), ") suggests:</p>",
                                "<ul>",
                            ...
                            paste0(
                                "<p> <strong>Homogeneous HER2 pattern</strong> (entropy = ", round(entropy_mean, 2), "):</p>",
                                "<ul>",
                    ...
                    biomarker_interpretation <- paste0(
                        ...
                        if (has_entropy && entropy_mean > 2.2) {
                            paste0(
                                "<p> <strong>High PD-L1 spatial variability</strong> may impact immunotherapy response prediction:</p>",
                                "<ul>",
                    ...
                    biomarker_interpretation <- paste0(
                        ...
                        if (highest_cv > 1.5) {
                            paste0(
                                "<p> <strong>Heterogeneous immune infiltration</strong> patterns detected:</p>",
                                "<ul>",
            ...
            .generateCopyReadySummary = function(n_cases, n_features, highest_cv, most_variable, complete_rate) {
                ...
                return(paste0(
                    ...
                    if (biomarker_context != "general" && biomarker_context != "custom") {
                        paste0(
                            "<li><strong>Biomarker Context:</strong> ",
            ...
            .detectMisuse = function(texture_data) {
                ...
                if (nrow(texture_matrix) >= 3) {
                    feature_cvs <- apply(texture_matrix, 2, function(x) {
                        if (sd(x, na.rm = TRUE) == 0 || mean(x, na.rm = TRUE) == 0) {
                            return(0)
                        }
