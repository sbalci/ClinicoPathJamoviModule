    ...
    private = list(
        ...
        .detectSpatialClusters = function(coords_data, listw) {
            ...
            if (length(significant_locations) > 0) {
                ...
                cluster_summary <- aggregate(cluster_data,
                    by = list(cluster_data$cluster_type),
        ...
        .performTemporalAnalysis = function(data, measurement, x_coord, y_coord, time_var) {
            ...
            for (i in seq_along(time_points)) {
                ...
                trend_direction <- if (change_from_baseline > 0.05) "Increasing" else if (change_from_baseline < -0.05) "Decreasing" else "Stable"
        ...
        .plotAutocorrFunction = function(image, ggtheme, theme, ...) {
            if (!self$options$autocorr_plots) {
                return()
            }
        ...
        .plotLISAMap = function(image, ggtheme, theme, ...) {
            if (!self$options$lisa_plots || !self$options$local_analysis) {
                return()
            }
            ...
            tryCatch(
                {
                    ...
                    listw <- private$.createSpatialWeights(
                        coords_data, self$options$spatial_weights,
