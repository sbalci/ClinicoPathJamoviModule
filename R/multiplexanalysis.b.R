...
multiplexanalysisClass <- if (requireNamespace("jmvcore", quietly = TRUE)) {
    R6::R6Class(
                ...
                for (i in seq_len(ncol(marker_data))) {
                    ...
                    if (!is.numeric(col_data)) {
                        return(list(
                            valid = FALSE,
                            message = paste0(
                                "<p style='color: red;'><strong>Validation Error:</strong> Marker variable '",
                    ...
                    if (any(col_data < 0, na.rm = TRUE)) {
                        return(list(
                            valid = FALSE,
                            message = paste0(
                                "<p style='color: orange;'><strong>Warning:</strong> Marker variable '",
            ...
            .pcaplot = function(image, ggtheme, theme, ...) {
                ...
                p <- ggplot2::ggplot(pca_data, ggplot2::aes(x = PC1, y = PC2)) +
                    ...
                    ggplot2::labs(
                        ...
                        subtitle = paste(
                            "PC1:", round(pca_data$Variance_PC1[1], 1), "% variance,",
                ...
                if (self$options$show_loadings && !is.null(loadings_data)) {
                    ...
                    p <- p +
                        ...
                        ggplot2::geom_text(
                            data = loadings_data,
                            ggplot2::aes(
                                x = PC1_loading * scale_factor * 1.1,
