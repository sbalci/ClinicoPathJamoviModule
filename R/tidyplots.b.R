    ...
    private = list(
        ...
        .generateHowToGuide = function() {
            ...
            guide_html <- paste0(
                ...
                "<ul>",
                "<li><strong>X Variable:</strong> The independent variable (categories, groups, or continuous predictor)</li>",
                "<li><strong>Y Variable:</strong> The dependent variable (outcome you're measuring)</li>",
                "<li><strong>Color Variable (optional):</strong> Group your data by a categorical variable</li>",
                "<li><strong>Facet Variable (optional):</strong> Create separate panels for each category</li>",
                "</ul>",
                "<h5>Step 2: Choose Plot Type</h5>",
                "<ul>",
                "<li><strong>Points/Scatter:</strong> Show relationship between two continuous variables</li>",
                "<li><strong>Box Plot:</strong> Compare distributions across groups (shows median, quartiles, outliers)</li>",
                "<li><strong>Violin Plot:</strong> Like box plots but shows full data distribution shape</li>",
                "<li><strong>Bar Chart:</strong> Compare means or counts across categories</li>",
                "<li><strong>Line Plot:</strong> Show trends over time or ordered categories</li>",
                "<li><strong>Histogram:</strong> See the frequency distribution of a single variable</li>",
                "</ul>",
                "<h5>Step 3: Add Statistical Overlays (Optional)</h5>",
                "<ul>",
                "<li><strong>Show Mean/Median:</strong> Add central tendency indicators</li>",
                "<li><strong>Show CI/SEM:</strong> Display uncertainty around estimates</li>",
                "<li><strong>Show P-Values:</strong> Add statistical significance tests</li>",
                "</ul>",
                "<h5>Step 4: Customize Appearance</h5>",
                "<ul>",
                "<li><strong>Color Schemes:</strong> Choose colorblind-friendly palettes</li>",
                "<li><strong>Themes:</strong> Apply pre-designed visual styles</li>",
                "<li><strong>Labels:</strong> Add custom titles and axis labels</li>",
                "</ul>",
                "<p style='margin-top: 15px; margin-bottom: 0;'><strong>Tip:</strong> Start with just X and Y variables, then add features gradually. Use the glossary (check 'Show Statistical Glossary') to understand statistical terms.</p>",
                "</div>"
            )
        ...
        .populateGlossary = function() {
            ...
            glossary_html <- paste0(
                ...
                "<dd>Sum of all values divided by count. Shows the typical value.</dd>",
                "<dt><strong>Median (Middle Value)</strong></dt>",
                "<dd>The middle number when data is sorted. Less affected by extreme values than mean.</dd>",
                "<dt><strong>Standard Error (SEM)</strong></dt>",
                "<dd>Indicates how much the sample mean might differ from the true population mean. Smaller = more precise.</dd>",
                "<dt><strong>Standard Deviation (SD)</strong></dt>",
                "<dd>Measures how spread out the data is. About 68% of data falls within 1 SD of the mean.</dd>",
                "<dt><strong>95% Confidence Interval (CI)</strong></dt>",
                "<dd>Range where we're 95% confident the true value lies. If CI doesn't cross zero/baseline, the effect is statistically significant.</dd>",
                "<dt><strong>P-value</strong></dt>",
                "<dd>Probability that the observed pattern could occur by random chance. Typically <0.05 is considered statistically significant.</dd>",
                "<dt><strong>Alpha (Transparency)</strong></dt>",
                "<dd>Controls how see-through plot elements are (0=invisible, 1=solid). Useful for overlapping points.</dd>",
                "<dt><strong>Faceting</strong></dt>",
                "<dd>Splitting the plot into multiple panels based on a grouping variable.</dd>",
                "</dl>",
                "</div>"
            )
        ...
        .generatePlotSummary = function() {
            ...
            tryCatch({
                ...
                # Generate readable plot type description
                ...
                summary_parts <- c(summary_parts,
                    sprintf("The analysis includes <strong>%d complete observations</strong>.", n_obs)
                )
                ...
                # Add statistical overlay info
                stats_shown <- c()
                if (self$options$showMean) stats_shown <- c(stats_shown, "mean values")
                if (self$options$showMedian) stats_shown <- c(stats_shown, "median values")
                if (self$options$showCI) stats_shown <- c(stats_shown, "95% confidence intervals")
                if (self$options$showSD) stats_shown <- c(stats_shown, "standard deviation")
                if (self$options$showPValue) stats_shown <- c(stats_shown, "p-values")

                if (length(stats_shown) > 0) {
                    summary_parts <- c(summary_parts,
                        sprintf("Statistical overlays shown: %s.",
                                paste(stats_shown, collapse = ", "))
                    )
                }
                ...
                # If data prep fails, hide summary
                self$results$summary$setVisible(FALSE)
            })
        }
    )
