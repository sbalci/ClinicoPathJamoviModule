    ...
    private = list(
            ...
            tryCatch(
                {
                    ...
                    desc_stats <- aggregate(dependent ~ within,
                        data = long_data,
                        function(x) {
                            c(
                                median = median(x),
        ...
        .populateAssumptionAssessment = function(data_prepared, friedman_result) {
            ...
            table$addRow(rowKey = 1, values = list(
                ...
                details = paste0(
                    data_prepared$n_subjects, " subjects with complete data across ",
            ...
            table$addRow(rowKey = 2, values = list(
                ...
                recommendation = if (n_unique >= 5) {
                    "Sufficient measurement granularity"
                } else {
                    "Few unique values; consider exact test method"
                }
            ))
            ...
            table$addRow(rowKey = 4, values = list(
                ...
                details = paste0(
                    "N = ", data_prepared$n_subjects, " subjects, k = ",
                    data_prepared$n_conditions, " conditions"
                ),
            ...
            table$addRow(rowKey = 5, values = list(
                ...
                recommendation = if (n_ties == 0) {
                    "No correction needed"
                } else {
                    "Average ranks used for ties (standard approach)"
                }
            ))
        },
        ...
        .populateClinicalInterpretation = function(friedman_result) {
            ...
            table$addRow(rowKey = 2, values = list(
                ...
                interpretation = paste0(
                    "The ", w_interp, " effect suggests ",
                    if (w_val < 0.3) {
                        "minimal practical differences between conditions"
                    } else if (w_val < 0.5) {
                        "meaningful but modest differences between conditions"
                    } else {
                        "substantial practical differences between conditions"
                    }
                ),
            ...
            table$addRow(rowKey = 3, values = list(
                ...
                finding = paste0(
                    "Highest mean rank: ", best_cond,
                    " (", round(max(rank_sums$mean_rank), 2), ")",
                    "; Lowest: ", worst_cond,
                    " (", round(min(rank_sums$mean_rank), 2), ")"
                ),
                interpretation = paste0(
                    best_cond, " received consistently higher ratings/values compared to ",
                    worst_cond
                ),
        ...
        .plotMeanRanks = function(image, ggtheme, theme, ...) {
            ...
            plot <- ggplot2::ggplot(
                ...
                ggplot2::labs(
                    ...
                    caption = paste(
                        "Dashed line = expected rank under H0 (",
                        round(expected, 2), ")"
                    )
                ) +
                ggplot2::theme_minimal() +
