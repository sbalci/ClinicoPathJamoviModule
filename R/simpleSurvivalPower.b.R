    ...
    private = list(
        ...
        .validate_parameter_combinations = function(result) {
            ...
            if (!is.null(median_survival) && !is.null(accrual) && !is.null(followup)) {
                ...
                if (total_duration < median_survival * 1.5) {
                    result$warnings <- append(
                        result$warnings,
                        paste0(
                            "Study duration (", round(total_duration, 1), " months) may be too short ",
            ...
            if (!is.null(ratio)) {
                if (ratio < 0.5 || ratio > 2) {
                    ...
                    result$warnings <- append(
                        result$warnings,
                        paste0(
                            "Allocation ratio ", round(ratio, 2), ":1 reduces efficiency by ",
            ...
            if (length(result$warnings) > 0) {
                warning_text <- paste0(
                    "<b>Clinical Considerations:</b><ul>",
        ...
        .populate_power_results = function() {
            ...
            parameters <- list(
                ...
                list(
                    parameter = "Expected Events", value = paste(expected_events, "events"),
        ...
        .populate_effect_size_results = function() {
            ...
            parameters <- list(
                list(
                    parameter = "Minimum Detectable HR", value = round(min_detectable_hr, 3),
                ...
                list(
                    parameter = "Risk Reduction", value = paste0(percent_reduction, "%"),
        ...
        .populate_duration_results = function() {
            ...
            parameters <- list(
                list(
                    parameter = "Required Duration", value = paste(total_duration, "months"),
                    description = "Total study duration needed to achieve target power"
                ),
                list(
                    parameter = "Accrual Period", value = paste(accrual_period, "months"),
                    description = "Patient recruitment period"
                ),
                list(
                    parameter = "Minimum Follow-up", value = paste(min_follow_up, "months"),
                    description = "Additional follow-up needed after recruitment"
                ),
                list(
                    parameter = "Required Events", value = paste(events_needed, "events"),
                    description = "Number of events needed for target power"
                ),
                list(
                    parameter = "50% Events Time", value = paste(round(fifty_percent_events_time, 1), "months"),
