qualitycontrolClass <- if (requireNamespace("jmvcore", quietly = TRUE)) {
    R6::R6Class(
        "qualitycontrolClass",
        ...
        private = list(
            ...
            .performControlChartsAnalysis = function(measurements, time_data, control_levels) {
                ...
                if (chart_type %in% c("shewhart", "all_charts")) {
                    ...
                    performance <- if (out_of_control == 0) "In Control" else if (out_of_control <= length(measurements) * 0.01) "Acceptable" else "Out of Control"
            ...
            .performSigmaMetricsAnalysis = function(measurements, control_levels) {
                ...
                quality_rating <- if (sigma_level >= 6) "World Class" else if (sigma_level >= 5) "Excellent" else if (sigma_level >= 4) "Good" else if (sigma_level >= 3) "Marginal" else "Poor"

                # Recommendations
                recommendation <- if (sigma_level >= 5) {
                    "Maintain current performance"
                } else if (sigma_level >= 4) {
                    "Minor improvements needed"
                } else if (sigma_level >= 3) {
            ...
            .performProficiencyTestingAnalysis = function(measurements) {
                ...
                for (i in seq_along(measurements)) {
                    ...
                    performance <- if (abs(z_score) <= 2.0) "Satisfactory" else if (abs(z_score) <= 3.0) "Questionable" else "Unsatisfactory"

                    interpretation <- if (abs(z_score) <= 2.0) {
                        "Within acceptable limits"
                    } else if (abs(z_score) <= 3.0) {
                ...
                performance_summary <- if (all(abs(z_scores) <= 2.0)) {
                    "All satisfactory"
                } else if (any(abs(z_scores) > 3.0)) {
                    "Some unsatisfactory"
                } else {
                    "Some questionable"
                }
