    ...
    R6::R6Class(
        ...
        private = list(
            ...
            .parseTimePoints = function(time_string) {
                if (is.null(time_string) || time_string == "") {
                    return(numeric(0))
                }
                ...
                if (!is.null(self$options$group)) {
                    ...
                    if (length(groups) == 2) {
                        for (i in seq_along(groups)) {
                            ...
                            overview_table$setCell(
                                rowKey = "n", col = col_name,
                                value = as.character(n_group)
                            )
                            overview_table$setCell(
                                rowKey = "events", col = col_name,
                                value = sprintf("%d (%.1f%%)", n_events_group, event_rate_group)
                            )
                            overview_table$setCell(
                                rowKey = "median_time", col = col_name,
            ...
            .interpretConditionalSurvival = function(cs_prob, pred_time, time_scale) {
                ...
                if (cs_pct >= 90) {
                    return(sprintf(
                        "Excellent prognosis: %.1f%% chance of surviving additional %.1f %s",
                        cs_pct, pred_time, time_scale
                    ))
                } else if (cs_pct >= 75) {
                    return(sprintf(
                        "Good prognosis: %.1f%% chance of surviving additional %.1f %s",
                        cs_pct, pred_time, time_scale
                    ))
                } else if (cs_pct >= 50) {
                    return(sprintf(
                        "Moderate prognosis: %.1f%% chance of surviving additional %.1f %s",
                        cs_pct, pred_time, time_scale
                    ))
                } else {
                    return(sprintf(
                        "Guarded prognosis: %.1f%% chance of surviving additional %.1f %s",
                        cs_pct, pred_time, time_scale
                    ))
                }
            },
                            ...
                            if (patient_time + horizon <= max(data$time)) {
                                ...
                                if (inherits(model, "survfit")) {
                                    ...
                                    if (!is.na(s_current) && s_current > 0) {
                                        ...
                                        # Risk categorization
                                        risk_cat <- if (dynamic_prob >= 0.8) "Low Risk" else if (dynamic_prob >= 0.6) "Moderate Risk" else "High Risk"
                        ...
                        tryCatch(
                            {
                                ...
                                significance <- if (p_value < 0.001) "p < 0.001" else if (p_value < 0.01) "p < 0.01" else if (p_value < 0.05) "p < 0.05" else "Not significant"
