        ...
        private = list(
            ...
            .validate_inputs = function() {
                ...
                if (is.null(self$options$targetLevel)) {
                    return(list(
                        valid = FALSE, show_welcome = FALSE,
            ...
            .parse_range = function(range_str, default_range) {
                ...
                if (any(is.na(vals)) || !all(is.finite(vals))) {
                    return(default_range)
                }
                vals
            },
                ...
                tryCatch(
                    {
                        ...
                        p <- ggplot2::ggplot(calib_data, ggplot2::aes(x = pred_prob, y = obs_prob)) +
                            ggplot2::geom_line(
                                data = perfect_line, ggplot2::aes(x = x, y = y),
                ...
                tryCatch(
                    {
                        ...
                        p <- ggplot2::ggplot(roc_data, ggplot2::aes(x = fpr, y = tpr)) +
                            ggplot2::geom_line(
                                data = diagonal, ggplot2::aes(x = x, y = y),
            ...
            .parttree_plot = function(image, ggtheme, ...) {
                ...
                if (!requireNamespace("parttree", quietly = TRUE)) {
                    ...
                    text(
                        x = 0.5, y = 0.5, paste("parttree package is required for this plot.\nPlease install it with:\nremotes::install_github('grantmcdermott/parttree')"),
                        cex = 1.0, col = "black"
                    )
                    return(TRUE)
                }
                ...
                tryCatch(
                    ...
                    error = function(e) {
                        plot(c(0, 1), c(0, 1), ann = F, bty = "n", type = "n", xaxt = "n", yaxt = "n")
                        text(
                            x = 0.5, y = 0.5, paste("Error generating parttree plot:\n", e$message),
                            cex = 1.0, col = "red"
                        )
                        return(TRUE)
                    }
