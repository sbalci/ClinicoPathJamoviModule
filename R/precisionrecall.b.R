...
#' @importFrom ggplot2 ggplot aes geom_line geom_hline geom_abline annotate scale_x_continuous scale_y_continuous labs theme_minimal theme element_blank
NULL
...
precisionrecallClass <- if (requireNamespace('jmvcore', quietly=TRUE)) R6::R6Class(
    ...
    private = list(
        ...
        .run = function() {
            ...
            # so the wrapper is no-op when neither bootstrap path runs.
            # Mirrors optimalcutpoint.b.R:765-772.
            if (isTRUE(self$options$ci) ||
                (isTRUE(self$options$comparison) && length(self$options$scores) > 1)) {
            ...
            if (length(unique(jmvcore::naOmit(outcome))) != 2) {
                jmvcore::reject("Outcome variable must have exactly 2 levels")
            }
