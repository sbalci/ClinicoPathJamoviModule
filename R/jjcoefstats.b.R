        ...
        .runFitModel = function() {
            ...
            tryCatch({
                if (model_type == "lm") {
                    ...
                    # Determine if outcome is continuous or binary
                    outcome_vals <- mydata[[outcome_var]]
                    is_binary <- length(unique(na.omit(outcome_vals))) == 2

                    if (is_binary) {
                        # Use glmer for binary outcome
                        model <- lme4::glmer(formula, data = mydata, family = binomial(link = "logit"))
                    } else {
                        # Use lmer for continuous outcome
                        model <- lme4::lmer(formula, data = mydata)
                    }
                } else {
