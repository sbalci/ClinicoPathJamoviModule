    ...
    R6::R6Class("datevalidatorClass",
        ...
        private = list(
            ...
            .classify_date_pattern = function(date_str) {
                ...
                if (grepl("^\\d{4}-\\d{2}-\\d{2}\\s+\\d{2}:\\d{2}:\\d{2}", date_str)) {
                    return("ISO datetime (YYYY-MM-DD HH:MM:SS)")
                }
                if (grepl("^\\d{4}-\\d{2}-\\d{2}$", date_str)) {
                    return("ISO date (YYYY-MM-DD)")
                }
                if (grepl("^\\d{1,2}/\\d{1,2}/\\d{4}\\s+\\d{1,2}:\\d{2}", date_str)) {
                    return("US datetime (MM/DD/YYYY HH:MM)")
                }
                if (grepl("^\\d{1,2}/\\d{1,2}/\\d{4}$", date_str)) {
                    return("US format (MM/DD/YYYY)")
                }
                if (grepl("^\\d{1,2}/\\d{1,2}/\\d{2}\\s+\\d{1,2}:\\d{2}", date_str)) {
                    return("Short US datetime (MM/DD/YY HH:MM)")
                }
                if (grepl("^\\d{1,2}/\\d{1,2}/\\d{2}$", date_str)) {
                    return("Short US (MM/DD/YY)")
                }
                if (grepl("^\\d{1,2}-\\d{1,2}-\\d{4}$", date_str)) {
                    return("Dash separated (DD-MM-YYYY)")
                }
                if (grepl("^\\d{4}/\\d{1,2}/\\d{1,2}$", date_str)) {
                    return("ISO with slash (YYYY/MM/DD)")
                }
                if (grepl("^\\d+$", date_str)) {
                    return("Numeric (Excel/Unix?)")
                }
                if (grepl("[A-Za-z]", date_str)) {
                    return("Text month")
                }
