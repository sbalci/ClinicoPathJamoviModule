...
concordanceindexClass <- if (requireNamespace("jmvcore", quietly = TRUE)) {
    R6::R6Class(
        ...
        private = list(
            ...
            .run = function() {
                ...
                if (self$options$competing_risks) {
                    private$.addNotice(
                        "ERROR", "Competing Risks Not Implemented",
                ...
                if (self$options$decompose_cindex) {
                    private$.addNotice(
                        "ERROR", "Decomposition Not Implemented",
                ...
                if (self$options$stratified_cindex) {
                    private$.addNotice(
                        "ERROR", "Stratified Analysis Not Implemented",
            ...
            .populateCindexSummary = function(time_var, event_var, predictor) {
                ...
                private$.addNotice("INFO", "Analysis Complete", sprintf(
                    "C-index analysis completed: %d observations, %d events (%.1f%% event rate), method=Harrell.",
            ...
            .calculateCindexCI = function(time_var, event_var, predictor, cindex) {
                ...
                if (self$options$ci_method == "bootstrap") {
                    ...
                    if (n_failed > 0.1 * n_boot) {
                        private$.addNotice("WARNING", "Bootstrap Failures", sprintf(
                            "%d/%d bootstrap samples failed (%.1f%%). Confidence intervals may be unreliable. Consider using asymptotic CI method or checking data quality.",
            ...
            .populateTimeDependentCindex = function(time_var, event_var, predictor) {
                ...
                # Warn users about limitations
                private$.addNotice(
                    "STRONG_WARNING", "Time-Dependent C-index Not Implemented",
                ...
                for (i in seq_along(eval_times)) {
                    ...
                    if (sum(include) < 10) {
                        private$.addNotice("WARNING", "Time-Dependent C-index Skipped", sprintf(
                            "Time-dependent C-index at time point %.1f skipped due to insufficient observations (n=%d < 10). Consider removing this time point or using shorter evaluation times.",
            ...
            .populatePairwiseTests = function(comparison_table) {
                ...
                private$.addNotice(
                    "STRONG_WARNING", "Pairwise Test Limitation",
            ...
            .plotModelComparison = function(image, ...) {
                ...
                table <- self$results$modelComparison
                if (is.null(table) || table$rowCount == 0) {
                    return()
                }
