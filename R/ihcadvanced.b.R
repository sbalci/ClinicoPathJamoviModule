        ...
        private = list(
            ...
            .performConsensusClustering = function() {
                ...
                for (k_idx in seq_along(k_range)) {
                    ...
                    stability <- if (avg_consensus > 0.85) {
                        "Very High"
                    } else if (avg_consensus > 0.75) {
                        "High"
                    } else if (avg_consensus > 0.6) {
                        "Moderate"
            ...
            .validateClusters = function() {
                ...
                metrics <- list(
                    list(
                        ...
                        interpretation = if (avg_silhouette > 0.7) {
                            "Strong structure"
                    ...
                    list(
                        metric = "Connectivity",
                        value = connectivity,
                        interpretation = if (connectivity < 10) {
                            "Compact clusters"
                        } else if (connectivity < 20) {
                    ...
                    list(
                        metric = "Dunn Index",
                        value = dunn_index,
                        interpretation = if (!is.na(dunn_index) && dunn_index > 0.5) {
                            "Well-separated"
                        } else if (!is.na(dunn_index) && dunn_index > 0.2) {
                            "Moderate separation"
                        } else {
                            "Poor separation"
                        },
            ...
            .generateClinicalSummary = function() {
                ...
                if (!is.null(private$.clusters)) {
                    ...
                    if (!is.null(private$.validation_results)) {
                        ...
                        if (!is.na(avg_sil)) {
                            sil_interpretation <- if (avg_sil > 0.7) {
            ...
            .plotOptimalK = function(image, ...) {
                ...
                if (self$options$optimalKMethod == "silhouette" && length(silhouette_values) > 0) {
                    ...
                    # Elbow plot
                    plot(k_values, within_ss_values,
                        type = "b",
                        main = "Elbow Method",
                        xlab = "Number of Clusters (K)",
            ...
            .plotSilhouette = function(image, ...) {
                ...
                plot(sil,
                    main = "Silhouette Plot - Cluster Quality Assessment",
                    col = sil_colors
                )
