...
clinicalvalidationClass <- R6::R6Class(
  ...
  private = list(
      ...
      tryCatch(
        {
          ...
          if (nchar(self$options$model_formula) > 0) {
            ...
            formula_str <- paste(
              "Surv(", jmvcore::composeTerm(time_var), ",", jmvcore::composeTerm(outcome_var), ") ~",
          ...
          model <- switch(self$options$model_type,
    ...
    .compareModels = function(prepared_data) {
      ...
      models_to_compare <- switch(comparison_type,
    ...
    .prc_curve_plot = function(image, ggtheme, theme, ...) {
      ...
      pr_data <- PRROC::pr.curve(
        scores.class0 = pred_probs[actual == 1],
