...
clinicaltrialdesignClass <- if (requireNamespace("jmvcore", quietly = TRUE)) {
    R6::R6Class(
            ...
            .generateClinicalInterpretation = function() {
                ...
                interpretation_text <- paste0(
                    ...
                    "<li><b>Interim Analyses:</b> ", if (self$options$interim_analyses > 0) {
                        ...
                        "No interim analyses planned - consider for long studies"
                    }, "</li>",
                    "</ul>",
            ...
            .generateProtocolTemplate = function() {
                ...
                protocol_text <- paste0(
                    ...
                    "<li>Effect size: ", if (self$options$outcome_type == "continuous") {
                        ...
                        paste("Proportion difference =", abs(self$options$proportion2 - self$options$proportion1))
                    }, "</li>",
            ...
            .interpretPowerResult = function(result, calc_type) {
                switch(calc_type,
                    "power" = {
                        power <- result$power
                        if (power >= 0.90) {
                            "Excellent power - high probability of detecting true effect"
                        } else if (power >= 0.80) {
                            "Adequate power - standard for clinical trials"
                        } else if (power >= 0.70) {
                            "Moderate power - consider increasing sample size"
                        } else {
                            "Low power - substantial risk of missing true effect"
                        }
                    },
                    "sample_size" = {
                        n <- result$n
                        if (n <= 50) {
                            "Small study - feasible but limited generalizability"
                        } else if (n <= 200) {
                            "Medium-sized study - good balance of feasibility and power"
                        } else if (n <= 1000) {
                            "Large study - high power but requires substantial resources"
                        } else {
                            "Very large study - consider adaptive design or surrogate endpoints"
                        }
                    },
                    "effect_size" = {
                        es <- result$effect_size
                        if (es >= 0.8) {
                            "Large detectable effect - may miss smaller but clinically important effects"
                        } else if (es >= 0.5) {
                            "Medium detectable effect - balanced sensitivity"
                        } else if (es >= 0.2) {
                            "Small detectable effect - high sensitivity to detect small changes"
                        } else {
                            "Very small detectable effect - excellent sensitivity"
                        }
                    }
                )
            },
            ...
            .assessClinicalSignificance = function(effect_size, outcome_type) {
                if (outcome_type == "continuous") {
                    ...
                    if (abs_d >= 0.5) {
                        ...
                        "May lack clinical significance - verify against MCID"
                    }
                } else {
                    if (effect_size >= 0.1) {
                        "Likely clinically meaningful"
                    } else {
                        "May lack clinical significance"
                    }
                }
            },
            .interpretNNT = function(nnt) {
                if (nnt <= 5) {
                    "Very beneficial - few patients needed to treat"
                } else if (nnt <= 10) {
