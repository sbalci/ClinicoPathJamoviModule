...
assayoptimizationClass <- R6::R6Class(
    ...
    private = list(
        .init = function() {
            ...
            safe_response <- htmltools::htmlEscape(response_var)
            safe_factors <- paste(vapply(factors, htmltools::htmlEscape, character(1)), collapse = ", ")
            self$results$instructions$setContent(
                paste0("<h3>Assay Optimization Analysis Ready</h3>
                <p><b>Response Variable:</b> ", safe_response, "</p>
                <p><b>Experimental Factors (", n_factors, "):</b> ", safe_factors, "</p>
        ...
        .run = function() {
            ...
            if (length(missing_factors) > 0) {
                safe_missing <- paste(vapply(missing_factors, htmltools::htmlEscape, character(1)), collapse = ", ")
                self$results$design_summary$setContent(
                    paste("Error: The following factors were not found in data:", safe_missing)
                )
                return()
            }
        ...
        .analyzeDesign = function(response, factor_data, response_var, factors) {
            ...
            for (i in seq_along(factors)) {
                ...
                safe_name <- htmltools::htmlEscape(factor_name)

                if (is.factor(factor_values)) {
                    factor_type <- "Categorical"
                    levels_info <- paste(vapply(levels(factor_values), htmltools::htmlEscape, character(1)), collapse = ", ")
                } else {
                ...
                html <- paste0(html, "<tr>")
                html <- paste0(html, "<td>", safe_name, "</td>")
        ...
        .performPowerAnalysis = function(response, factor_data) {
            ...
            tryCatch({
                ...
                html <- paste0(html, "<p>Power analysis error: ", htmltools::htmlEscape(e$message), "</p>")
            })
            ...
            tryCatch({
                ...
                if (exists("optimal_idx")) {
                    ...
                    for (factor_name in factors) {
                        safe_name <- htmltools::htmlEscape(factor_name)
                        html <- paste0(html, "<tr><td><b>", safe_name, ":</b></td><td>",
                                     round(jmvcore::toNumeric(optimal_conditions[[factor_name]]), 4), "</td></tr>")
                    }
                ...
                html <- paste0(html, "<p>Optimization error: ", htmltools::htmlEscape(e$message), "</p>")
            })
        ...
        .analyzeFactorEffects = function(response, factor_data, response_var, factors) {
            ...
            tryCatch({
                ...
                formula_str <- jmvcore::constructFormula(response_var, as.list(factors))
                model <- aov(jmvcore::asFormula(formula_str), data = model_data)
                ...
                for (i in 1:(nrow(anova_result) - 1)) {  # Exclude residuals row for significance
                    ...
                    html <- paste0(html, "<tr>")
                    html <- paste0(html, "<td>", htmltools::htmlEscape(source), "</td>")
                ...
                for (i in 1:(nrow(anova_result) - 1)) {
                    ...
                    html <- paste0(html, "<tr>")
                    html <- paste0(html, "<td>", htmltools::htmlEscape(factor_name), "</td>")
                ...
                html <- paste0(html, "<p>Factor effects analysis error: ", htmltools::htmlEscape(e$message), "</p>")
            })
        ...
        .performResponseSurface = function(response, factor_data, response_var, factors) {
            ...
            tryCatch({
                ...
                escaped_factors <- vapply(factors, jmvcore::composeTerm, character(1))
                linear_terms <- paste(escaped_factors, collapse = " + ")
                quadratic_terms <- paste0("I(", escaped_factors, "^2)", collapse = " + ")

                if (length(factors) >= 2) {
                    interaction_terms <- combn(factors, 2, FUN = function(x) jmvcore::composeTerm(x))
                    interaction_str <- paste(interaction_terms, collapse = " + ")
                    formula_str <- paste(jmvcore::composeTerm(response_var), "~", linear_terms, "+", quadratic_terms, "+", interaction_str)
                } else {
                    formula_str <- paste(jmvcore::composeTerm(response_var), "~", linear_terms, "+", quadratic_terms)
                }

                rs_model <- lm(jmvcore::asFormula(formula_str), data = model_data)
                ...
                if (length(factors) == 2) {
                    ...
                    if (!any(is.na(c(b1, b2, b11, b22, b12)))) {
                        ...
                        if (abs(det_B) > 1e-10) {
                            ...
                            safe_f1 <- htmltools::htmlEscape(factors[1])
                            safe_f2 <- htmltools::htmlEscape(factors[2])
                            html <- paste0(html, "<h4>Stationary Point Analysis</h4>")
                            html <- paste0(html, "<table class='jamovi-table'>")
                            html <- paste0(html, "<tr><td><b>", safe_f1, " (stationary):</b></td><td>", round(x1_stat, 4), "</td></tr>")
                            html <- paste0(html, "<tr><td><b>", safe_f2, " (stationary):</b></td><td>", round(x2_stat, 4), "</td></tr>")
                ...
                html <- paste0(html, "<p>Response surface analysis error: ", htmltools::htmlEscape(e$message), "</p>")
            })
        ...
        .performQualityControl = function(response, factor_data, response_var) {
            ...
            tryCatch({
                ...
                html <- paste0(html, "<p>Quality control analysis error: ", htmltools::htmlEscape(e$message), "</p>")
            })
        ...
        .performMethodValidation = function(response, factor_data, response_var, factors) {
            ...
            tryCatch({
                ...
                if (any(continuous_factors)) {
                    ...
                    html <- paste0(html, "<h4>Linearity Assessment (", htmltools::htmlEscape(cont_factor_name), ")</h4>")
                    html <- paste0(html, "<table class='jamovi-table'>")
                ...
                if (length(factors) > 0) {
                    ...
                    if (length(significant_factors) > 0) {
                        safe_sig <- paste(vapply(significant_factors, htmltools::htmlEscape, character(1)), collapse = ", ")
                        html <- paste0(html, "<tr><td><b>Critical Factors:</b></td><td>", safe_sig, "</td></tr>")
                    }
                ...
                html <- paste0(html, "<p>Method validation error: ", htmltools::htmlEscape(e$message), "</p>")
            })
