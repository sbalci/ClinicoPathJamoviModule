    ...
    R6::R6Class(
        ...
        private = list(
            ...
            .getOriginalName = function(safe_name) {
                if (is.null(private$.originalVarNames)) {
                    return(safe_name)
                }
            ...
            .run = function() {
                ...
                <br><hr>"
                    ...
                    if (!is.null(vars) && length(vars) > 0 && length(vars) < 3) {
                        private$.addNotice(
                            ...
                            sprintf(
                                "PCA requires at least 3 variables. Currently selected: %d. Please add %d more variable(s).",
                    ...
                    if (length(missing_vars) > 0) {
                        private$.addNotice(
                            ...
                            sprintf(
                                "The following variables are not in the dataset: %s. Please check variable selection and ensure all selected variables exist.",
                    ...
                    if (length(non_numeric) > 0) {
                        private$.addNotice(
                            "ERROR", "Non-Numeric Variables",
                            sprintf(
                                "PCA requires numeric variables. Non-numeric variables detected: %s. Please select only continuous numeric variables.",
                ...
                if (n_obs < 20) {
                    private$.addNotice(
                        "STRONG_WARNING", "Very Small Sample Size",
                        sprintf(
                            "Very small sample size (%d observations). PCA with <20 observations is exploratory only and results may not generalize.",
                            n_obs
                        )
                    )
                }
                ...
                if (!isTRUE(self$options$center)) {
                    private$.addNotice(
                        "STRONG_WARNING", "Centering Disabled",
                ...
                if (self$options$ncomp > ncomp_available) {
                    private$.addNotice(
                        "WARNING", "Components Truncated",
                        sprintf(
                            "Requested %d components but only %d are available (limited by min of variables, observations). Displaying PC1-PC%d.",
                ...
                if (row_info$rows_removed > 0) {
                    private$.addNotice(
                        "INFO", "Missing Data Removed",
                        sprintf(
                            "Removed %d row(s) with missing values. Analysis uses %d complete observations.",
            ...
            .runPCA = function() {
                ...
                if (nrow(pca_matrix) < 3) {
                    private$.addNotice(
                        ...
                        sprintf(
                            "Insufficient data for PCA. Found %d complete observation(s) after removing missing values. Need at least 3 observations. Check for extensive missing data in selected variables.",
                            nrow(pca_matrix)
                        )
                    )
                    return()
            ...
            .scree = function(image, ggtheme, theme, ...) {
                ...
                s_plot <- ggplot(var_df, aes(
                    x = factor(component, levels = component),
                    y = variance
                )) +
                    ...
                    labs(
                        x = "Component", y = "Variance explained (proportion)",
