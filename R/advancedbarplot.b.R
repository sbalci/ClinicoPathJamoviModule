        ...
        private = list(
            ...
            .run = function() {
                ...
                tryCatch(
                    ...
                    error = function(e) {
                        # Note: this tryCatch catches the jmvcore::reject() condition raised in
                        # .validate_inputs and re-routes it to setContent, which means the user
                        # never sees jamovi's native error UI. If you want reject to surface
            ...
            .process_data = function() {
                ...
                # The same pattern recurs at L531 (.add_statistical_overlay path).
                # Skipping the migration today only because the catalog targets literal na.omit
                # calls; revisit if downstream code starts depending on preserved attributes.
            ...
            .generate_summary_stats = function() {
                ...
                summary_html <- paste0(summary_html, "<th>", htmltools::htmlEscape(x_var), "</th>")
                ...
                if (stat_type == "count") {
                    ...
                    for (i in 1:length(freq_data)) {
                        ...
                        summary_html <- paste0(summary_html, "<td><strong>", htmltools::htmlEscape(category), "</strong></td>")
                ...
                tryCatch(
                    {
                        ...
                        if (stat_method == "anova") {
                            # One-way ANOVA
                            formula_str <- jmvcore::constructFormula(terms = x_var, dep = y_var)
                            aov_result <- aov(jmvcore::asFormula(formula_str), data = selected_data)
                    ...
                    error = function(e) {
                        error_msg <- paste("Error in statistical analysis:", htmltools::htmlEscape(e$message))
            ...
            .generate_code_example = function() {
                ...
                # Escape user-controlled column names before embedding in <code> block.
                ...
                tryCatch(
                    {
                        if (private$.safe_require_package("plotly", "Interactive plots") &&
                            ...
                            interactive_plot <- interactive_plot %>%
                                plotly::layout(
                                    title = list(text = paste("Interactive:", htmltools::htmlEscape(self$options$plot_title))),
                                    xaxis = list(title = htmltools::htmlEscape(self$options$x_title %||% x_var)),
                                    yaxis = list(title = htmltools::htmlEscape(self$options$y_title %||% y_column))
                                )
