...
ihcpredictClass <- if (requireNamespace("jmvcore", quietly = TRUE)) {
    R6::R6Class(
        "ihcpredictClass",
        ...
        private = list(
            ...
            <ul>
                <li><strong>High (>=0.75):</strong> Strong evidence, matches known patterns \u{2192} Accept diagnosis</li>
                <li><strong>Moderate (0.50-0.74):</strong> Reasonable evidence \u{2192} Consider confirmatory testing</li>
                <li><strong>Low (0.25-0.49):</strong> Weak evidence \u{2192} Recommend molecular testing</li>
                <li><strong>Very Low (<0.25):</strong> Insufficient evidence \u{2192} Mandatory molecular testing</li>
            </ul>
            ...
            .populateExportSummary = function() {
                ...
                lines <- c(
                    ...
                    sprintf(
                        "  \u{2022} %d cases require molecular testing (confidence < 0.25)",
                        sum(predictions$confidence < 0.25)
                    ),
                    sprintf(
                        "  \u{2022} %d cases should consider molecular testing (confidence 0.25-0.50)",
                        sum(predictions$confidence >= 0.25 & predictions$confidence < 0.50)
                    ),
                    "  \u{2022} All low-confidence cases should be discussed at tumor board",
                    "",
            ...
            .plotConfidenceDistribution = function(image, ggtheme, theme, ...) {
                ...
                p <- ggplot2::ggplot(predictions, ggplot2::aes(x = confidence)) +
                    ...
                    ggplot2::geom_vline(
                        xintercept = opts$highConfidenceThreshold %||% 0.75,
                        linetype = "dashed", color = "darkgreen", linewidth = 1
                    ) +
                    ggplot2::geom_vline(
                        xintercept = opts$lowConfidenceThreshold %||% 0.25,
                    ...
                    ggplot2::annotate("text",
                        x = opts$highConfidenceThreshold %||% 0.75, y = Inf,
                        label = "High Confidence", vjust = 2, hjust = 1.1, color = "darkgreen"
                    ) +
                    ggplot2::annotate("text",
                        x = opts$lowConfidenceThreshold %||% 0.25, y = Inf,
