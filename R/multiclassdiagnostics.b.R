...
multiclassdiagnosticsClass <- R6::R6Class(
    ...
    private = list(
        ...
        .run = function() {
            ...
            # read. Intended to trigger an export of the computed tables (confusionMatrix,
            # perClassMetrics, overallMetrics, modelComparison) to disk. If implemented:
            # constrain output path to a sandbox dir, prefer `jmvcore::Output`/result item
            # exporters over raw `writeLines`/`saveRDS`, and never accept a user-supplied
            # `path` option without `normalizePath` + prefix-check.
        },
        ...
        .compareModels = function(predicted1, predicted2, actual) {
            ...
            if (self$options$deLongTest && length(unique(actual)) == 2) {
                if (requireNamespace("pROC", quietly = TRUE)) {
                    ...
                    # TODO (correctness/stub): alphabet-derived binary encoding for DeLong test.
                    ...
                    #   pos <- self$options$positiveClass
                    #   if (is.null(pos) || ! pos %in% levels(actual))
                    #       jmvcore::reject("Select the positive class level for DeLong test.")
                    #   actual_binary <- as.numeric(actual == pos)
                    #   pred1_binary  <- as.numeric(predicted1 == pos)
                    #   pred2_binary  <- as.numeric(predicted2 == pos)
                    # Sibling functions (modelval, modelbuilder, mlpathology) use this pattern.
