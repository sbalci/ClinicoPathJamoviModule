    ...
    private = list(
        ...
        .validateData = function() {
            ...
            if (!timeVar %in% names(data))
                jmvcore::reject("Time variable {} not found in data", code = NULL, timeVar)
            if (!stageVar %in% names(data))
                jmvcore::reject("Stage variable {} not found in data", code = NULL, stageVar)
            if (!treatmentVar %in% names(data))
                jmvcore::reject("Treatment variable {} not found in data", code = NULL, treatmentVar)
            if (!patientId %in% names(data))
                jmvcore::reject("Patient ID variable {} not found in data", code = NULL, patientId)

            # Check for longitudinal structure
            timePoints <- length(unique(data[[timeVar]]))
            if (timePoints < 2)
                jmvcore::reject("Data must contain at least 2 time points for pathway analysis")

            patients <- length(unique(data[[patientId]]))
            if (patients < 5)
                jmvcore::reject("Data must contain at least 5 patients for meaningful analysis")

            # Check for missing values in key variables
            if (any(is.na(data[[timeVar]])))
                jmvcore::reject("Time variable cannot contain missing values")
            if (any(is.na(data[[patientId]])))
                jmvcore::reject("Patient ID variable cannot contain missing values")
