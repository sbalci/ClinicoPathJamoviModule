rpasurvivalClass <- if (requireNamespace("jmvcore", quietly = TRUE)) {
    R6::R6Class(
        "rpasurvivalClass",
        ...
        private = list(
            ...
            .run = function() {
                ...
                if (eventsPerPredictor < 10) {
                    private$.addNotice(
                        "WARNING", "Low Events-Per-Variable Ratio",
                        paste0(
                            "Only ", round(eventsPerPredictor, 1), " events per predictor. ",
                ...
                if (nPredictors > nEvents / 10) {
                    ...
                    private$.hideResults()
                    private$.addNotice(
                        "ERROR", "Severe Overfit Risk",
                        paste0(
                            "You have ", nPredictors, " predictors but only ", nEvents, " events. ",
                ...
                if (self$options$minbucket < 10) {
                    private$.addNotice(
                        "WARNING", "Very Small Node Size",
                        paste0(
                            "Minimum node size (", self$options$minbucket,
                ...
                if (self$options$maxdepth > 5) {
                    private$.addNotice(
                        "WARNING", "Very Deep Tree",
                        paste0(
                            "Maximum depth (", self$options$maxdepth,
                ...
                if (sum(!completeIdx) > 0) {
                    private$.addNotice(
                        ...
                        paste0(
                            sum(!completeIdx), " rows with missing data excluded (",
                ...
                if (self$options$prunetree && self$options$nfolds > 0) {
                    ...
                    private$.addNotice(
                        "INFO", "Tree Pruned",
                        paste0(
                            "Tree pruned using ", self$options$nfolds, "-fold cross-validation. Optimal CP = ",
                            round(optimalCP, 4), ". This helps prevent overfitting."
                        )
                    )
                } else if (!self$options$prunetree) {
                    private$.addNotice(
                        "WARNING", "Tree Not Pruned",
                ...
                if (self$options$variableimportance) {
                    if (!is.null(tree$variable.importance)) {
                        ...
                        private$.addNotice(
                            "INFO", "Variable Importance Unavailable",
                ...
                private$.addNotice(
                    ...
                    paste0(
                        "RPA identified ", nGroups, " risk groups from ", nPredictors, " predictor(s). ",
                ...
                if (self$options$showSummary) {
                    ...
                    for (i in 1:nGroups) {
                        summary_html <- paste0(
                            summary_html,
                            sprintf(
                                '<tr style="border-bottom: 1px solid #ddd;"><td><b>%s</b></td><td>%d</td><td>%d</td><td>%.1f %s</td><td>%s</td></tr>',
                                labels[i],
                                riskSummary$n[i],
                    ...
                    for (i in 1:nGroups) {
                        if (i == 1) {
                            report_text <- paste0(
                                report_text,
                                sprintf(
                                    "%s (n = %d) had a median overall survival of %.1f %s",
                                    labels[i],
                                    riskSummary$n[i],
                                    riskSummary$medianos[i],
                                    self$options$time_unit
