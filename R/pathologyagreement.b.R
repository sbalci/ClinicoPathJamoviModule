        ...
        private = list(
            ...
            .blandaltmanplot = function(image, ggtheme, theme, ...) {
                ...
                if (use_regression) {
                    ...
                    p <- ggplot2::ggplot(data, ggplot2::aes(x = Means, y = Differences)) +
                        ggplot2::geom_point(alpha = 0.6, color = "steelblue") +
                        ggplot2::geom_hline(
                            yintercept = unique(data$MeanDiff),
                            color = "black", linewidth = 1
                        ) +
                        ggplot2::geom_hline(
                            yintercept = unique(data$LoA_Lower),
                            color = "red", linetype = "dashed"
                        ) +
                        ggplot2::geom_hline(
                            yintercept = unique(data$LoA_Upper),
                            color = "red", linetype = "dashed"
                        ) +
                        ggplot2::geom_smooth(
                            method = "lm", se = TRUE,
            ...
            .generateInterpretation = function(method1, method2, validation_warnings = "") {
                ...
                interpretation <- paste0(
                    ...
                    " (", ifelse(abs(spearman_r) >= 0.90, "very strong",
                        ifelse(abs(spearman_r) >= 0.70, "strong", "moderate")
                    ), " correlation)</li>",
                    if (!is.na(icc_value)) {
                        paste0(
                            "<li><strong>Reliability:</strong> ICC(3,1) = ", round(icc_value, 3),
                            " (", ifelse(icc_value >= 0.90, "excellent",
                                ifelse(icc_value >= 0.75, "good", "moderate")
                            ), " reliability)</li>"
                        )
                    } else {
                        ""
            ...
            .generatePlainLanguageSummary = function(method1, method2) {
                ...
                summary_text <- sprintf(
                    ...
                    spearman_r,
                    bias_description,
                    if (agreement_level == "excellent agreement") {
                        "highly suitable for interchangeable"
                    } else if (agreement_level == "good agreement") {
                        "generally suitable for interchangeable"
                    } else if (agreement_level == "moderate agreement") {
                        "potentially suitable with caution for"
                    } else {
                        "not recommended for interchangeable"
                    }
                )
            ...
            .generateReportSentences = function(method1, method2) {
                ...
                icc_sentence <- if (!is.na(icc_value)) {
                    sprintf(
                        ...
                        if (icc_value >= private$.CLINICAL_CONSTANTS$ICC_EXCELLENT) {
                            "excellent"
                        } else if (icc_value >= private$.CLINICAL_CONSTANTS$ICC_GOOD) {
                            "good"
                        } else {
                            "moderate"
                        },
                ...
                conclusion_sentence <- sprintf(
                    ...
                    if (!is.na(icc_value) && icc_value >= private$.CLINICAL_CONSTANTS$ICC_GOOD &&
                        ...
                        "limited agreement not recommended for interchangeable use"
                    },
                    preset_context
                )
