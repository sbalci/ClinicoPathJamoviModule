...
factoranalysisClass <- R6::R6Class(
    ...
    private = list(
                ...
                tryCatch(
                    {
                        ...
                        interpretation <- if (overall_kmo >= 0.9) {
                            "Excellent"
        ...
        .populateParallelAnalysis = function() {
            if (is.null(private$.parallelResults)) {
                return()
            }
        ...
        .populateCommunalities = function() {
            if (!self$options$communalities || is.null(private$.faResult)) {
                return()
            }
        ...
        .populateFactorCorrelations = function() {
            if (!self$options$factorCorrelations || is.null(private$.faResult)) {
                return()
            }
            if (!self$options$rotation %in% c("oblimin", "promax")) {
                return()
            }
        ...
        .populateGoodnessOfFit = function() {
            if (self$options$method != "ml" || is.null(private$.faResult)) {
                return()
            }
        ...
        .populateFactorScoresStats = function() {
            if (self$options$scores == "none" || is.null(private$.faResult)) {
                return()
            }
        ...
        .populateComprehensiveResults = function() {
            if (!self$options$comprehensive_output || is.null(private$.faResult)) {
                return()
            }
            ...
            measures <- list(
                list(
                    "Total Variance Explained", paste(round(sum(private$.faResult$Vaccounted["Proportion Var", ]) * 100, 1), "%"),
                    "Percentage of total variance explained by extracted factors"
                ),
                list(
                    "Fit Statistic", ifelse(is.null(private$.faResult$fit), "N/A", round(private$.faResult$fit, 3)),
                    "Overall model fit index"
                ),
                list(
                    "Number of Observations", nrow(self$data[private$.vars]),
                    "Sample size used in analysis"
                ),
                list(
                    "Number of Variables", length(private$.vars),
        ...
        .plotScree = function(image, ggtheme, theme, ...) {
            if (is.null(private$.eigenvalues)) {
                return()
            }
