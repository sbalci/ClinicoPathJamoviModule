        ...
        private = list(
            ...
            .checkRequiredPackages = function(packages) {
                ...
                if (length(missing) > 0) {
                    jmvcore::reject(sprintf(
                        "Required packages not available: %s. Please install using: install.packages(c('%s'))",
            ...
            .validateTestConfiguration = function(tests, testLevels, mydata, goldVariable, goldPositive) {
                ...
                if (any(gold_table < 5)) {
                    validation_errors <- c(
                        validation_errors,
                        sprintf(
                            "Insufficient cases in outcome categories (minimum 5 per group recommended). Current: %s",
                ...
                if (any(high_missing)) {
                    validation_errors <- c(
                        validation_errors,
                        sprintf(
                            "High missing data rates in tests: %s",
                ...
                for (i in seq_along(tests)) {
                    ...
                    if (min_freq < 0.05) {
                        validation_errors <- c(
                            validation_errors,
                            sprintf(
                                "Test '%s' is extremely unbalanced (smallest group: %.1f%%)",
                                test, min_freq * 100
                            )
                        )
                    }
            ...
            .run = function() {
                ...
                private$.validateTestConfiguration(
                    testVariables, testPositiveLevels,
                ...
                for (i in seq_along(testVariables)) {
                    ...
                    if (!positive_level %in% levels(mydata[[test]])) {
                        ...
                        validation_errors <- c(
                            validation_errors,
                            paste0(
                                "Test '", test, "': Level '", positive_level,
                    ...
                    tryCatch(
                        {
                            ...
                            chunks <- split(
                                seq_along(all_test_combinations),
                ...
                if (!use_parallel) {
                    ...
                    for (batch_idx in 1:n_batches) {
                        ...
                        for (combo_idx in seq_along(batch_combinations)) {
                            ...
                            cache_key <- private$.getCacheKey(
                                tests, strategy, goldVariable, goldPositive,
                    ...
                    if (!is.null(self$options$goldPositive) && self$options$goldPositive != "") {
                        ...
                        if (self$options$goldPositive %in% gold_levels) {
                            ...
                            jmvcore::reject(paste(
                                "Gold positive level '", self$options$goldPositive,
                    ...
                    for (fold in 1:n_folds) {
                        ...
                        tryCatch(
                            ...
                            error = function(e) {
                                ...
                                warning(sprintf(
                                    "Cross-validation failed for panel '%s' fold %d: %s. Using original performance estimates.",
            ...
            .performBootstrap = function(mydata, panels, testVariables,
                            ...
                            tryCatch(
                                ...
                                error = function(e) {
                                    ...
                                    if (b <= 5) { # Only warn for first few failures to avoid spam
                                        warning(sprintf(
                                            "Bootstrap sample %d failed for panel '%s': %s. Using original estimates.",
                ...
                private$.updateProgress(
                    n_panels_to_bootstrap, n_panels_to_bootstrap,
            ...
            .populateAllCombinationsTable = function(combinations) {
                ...
                # Sort by accuracy
                sorted_combos <- combinations[order(sapply(
                    combinations,
            ...
            .populateTreeResults = function(tree_results) {
                if (is.null(tree_results$structure)) {
                    return()
                }
            ...
            .generateClinicalInterpretation = function(optimal_panels, testVariables, goldVariable, prevalence) {
                ...
                interpretation_html <- paste0(
                    ...
                    } else if (best_panel$sensitivity > 0.8) {
                        "This panel has good sensitivity for diagnostic use. "
                    } else {
                        "Consider additional tests if high sensitivity is critical. "
                    },
                    if (best_panel$specificity > 0.9) {
                        "The high specificity minimizes false positive results."
                    } else if (best_panel$specificity > 0.8) {
                        "The specificity is adequate for most clinical applications."
                    } else {
                ...
                if (length(outcome_balance) == 2) {
                    ...
                    if (balance_ratio < 0.1) {
                        warnings <- c(warnings, sprintf(
                            "Highly imbalanced outcome (%.1f%% vs %.1f%%) may affect performance estimates",
                    ...
                    if (min(outcome_balance) < 10) {
                        warnings <- c(warnings, sprintf(
                            "Very few cases in minority class (n = %d) - results may be unstable",
                            min(outcome_balance)
                        ))
                    }
                }
            ...
            .createSummary = function(optimal_panels, individual_results,
                ...
                html <- paste0(html, sprintf(
                    "<p>Number of tests evaluated: %d</p>",
            ...
            .createRecommendations = function(optimal_panels, individual_results,
                ...
                if (length(optimal_panels) > 0) {
                    ...
                    html <- paste0(html, "<ul>")
                    html <- paste0(
                        html, "<li><strong>", htmltools::htmlEscape(best$tests), "</strong> using <strong>",
                    ...
                    if (grepl("parallel_any", best$strategy)) {
                        html <- paste0(
                            html, "<p>This means conducting all tests simultaneously and ",
                ...
                if (length(optimal_panels) > 1) {
                    ...
                    high_sens <- optimal_panels[order(sapply(
                        optimal_panels,
                        function(x) -x$sensitivity
                    ))][[1]]
                    html <- paste0(html, sprintf(
                        "<li>For maximum sensitivity (%.1f%%): %s</li>",
                        high_sens$sensitivity * 100, htmltools::htmlEscape(high_sens$tests)
                    ))

                    # High specificity option
                    high_spec <- optimal_panels[order(sapply(
                        optimal_panels,
                        function(x) -x$specificity
                    ))][[1]]
                    html <- paste0(html, sprintf(
                        "<li>For maximum specificity (%.1f%%): %s</li>",
                        high_spec$specificity * 100, htmltools::htmlEscape(high_spec$tests)
                    ))

                    # Low cost option (if applicable)
                    if (self$options$useCosts) {
                        low_cost <- optimal_panels[order(sapply(
                            optimal_panels,
                            function(x) x$cost
                        ))][[1]]
                        html <- paste0(html, sprintf(
                            "<li>For minimum cost (%.2f): %s</li>",
                ...
                html <- paste0(html, "<ul>")
                html <- paste0(
                    html, "<li>Consider the clinical context and consequences of ",
                    "false positives vs. false negatives</li>"
                )
                html <- paste0(
                    html, "<li>Factor in practical considerations such as test ",
                    "availability and turnaround time</li>"
                )
                html <- paste0(
                    html, "<li>Validate the chosen panel in an independent dataset ",
            ...
            .findOptimalPanelsBySize = function(mydata, testVariables, goldVariable,
                ...
                if (total_combinations > 10000) {
                    message(sprintf(
                        "Large search space detected (%d combinations). Consider reducing maxTests or using heuristic optimization.",
                        total_combinations
                    ))
                ...
                for (panel_size in 1:max_panel_size) {
                    ...
                    for (combo_idx in seq_along(combinations)) {
                        ...
                        if (best_performance > 0.95 && panel_size >= 2) {
                            message(sprintf(
                                "Early stopping for size %d: Excellent performance achieved (%.3f)",
                        ...
                        if (combinations_evaluated > 500 && combo_idx > length(combinations) * 0.5) {
                            ...
                            if (recent_improvements == 0) {
                                message(sprintf(
                                    "Early stopping for size %d: No recent improvements after %d evaluations",
            ...
            .testImprovement = function(mydata, tests_new, tests_old,
                ...
                if ((correct_old_wrong_new + wrong_old_correct_new) > 0) {
                    test_result <- stats::mcnemar.test(
                        matrix(c(
                            correct_old_wrong_new, wrong_old_correct_new,
                            wrong_old_correct_new, correct_old_wrong_new
                        ), 2, 2)
                    )
            ...
            .populatePanelBuildingTable = function(panel_optimization) {
                ...
                if (length(panel_optimization) > 0) {
                    table$addFootnote(
                        rowNo = 1, col = "improvementPercent",
            ...
            .validateInputs = function() {
                ...
                test_count <- sum(!is.null(c(
                    self$options$test1, self$options$test2,
                ...
                if (total_requests > 0) {
                    ...
                    return(list(
                        ...
                        efficiency_message = sprintf(
                            "Cache efficiency: %.1f%% hit rate (%d/%d requests)",
            ...
            .plotCostEffectiveness = function(image, ggtheme, theme, ...) {
                ...
                pareto_indices <- which(!duplicated(plot_data[
                    order(
                        plot_data$cost,
                        -plot_data$accuracy
                    ),
