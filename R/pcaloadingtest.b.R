...
through <b>Procrustes rotation</b> to handle sign/reflection indeterminacy in PCA solutions.
            ...
            .runPermutationTest = function() {
                ...
                if (epv_ratio < 10) {
                    private$.addNotice("STRONG_WARNING", "Low Sample-to-Variable Ratio", sprintf(
                        "Sample size to variable ratio is low (N=%d, variables=%d, ratio=%.1f:1). PCA loadings may be unstable with low sample-to-variable ratios. Recommend ratio >= 10:1 (N >= %d for %d variables).",
                ...
                if (total_var_explained < 0.50) {
                    private$.addNotice("WARNING", "Low Variance Explained", sprintf(
                        "First %d components explain only %.1f%% of total variance. Consider whether PCA is appropriate for this data or increase the number of components tested. Low variance explained suggests weak underlying structure.",
                        ...
                        tryCatch(
                            {
                                ...
                                if (requireNamespace("pracma", quietly = TRUE)) {
                                    ...
                                    r <- pracma::procrustes(
                                        as.matrix(original_loadings[, 1:ndim]),
