...
ssgUNIClass <- if (requireNamespace('jmvcore')) R6::R6Class(
  ...
  private = list(
    .run = function() {
      ...
      y1 <- self$options$y1
      y2 <- paste0(y1,".lag",self$options$ssgUNIlag)
      ...
      if(sum(idNA1)>0){data <- data[idNA1,]}
      ...
      y2vec <- y1vec[self$options$ssgUNIlag:length(y1vec)]
      y1vec <- y1vec[1:(length(y1vec)-self$options$ssgUNIlag)]

      v1 <- as.numeric_discrete(y1vec, sortUnique = TRUE)
      v2 <- as.numeric_discrete(y2vec, sortUnique = TRUE)
      ...
      v2_Nstates_usr <- self$options$v1_Nstates
      v2_Nlabs_usr   <- length(self$options$v1_labels)
      #y2_ready <- TRUE

      if(v2_Nlabs_usr>0){
        v2_labs        <- self$options$v1_labels
        v2_labs_usr    <- strsplit(x = v1_labs, split = "[,]")[[1]]
        v2_Nlabs_usr <- length(v1_labs_usr)
      } else {
        v2_labs_usr <- ""
        v2_Nlabs_usr <- 0
      }
    ...
    .SSGplot=function(ssgImage, ...) {
      ...
      if(ready){
        ...
        ssgr <- ggplot2::ggplot(durData, ggplot2::aes(x=y1,
          ...
          ggplot2::scale_y_discrete(paste(y2),breaks = COORD_v2, labels = LABS_v2) +
