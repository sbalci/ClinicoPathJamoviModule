...
scat2Class <- if (requireNamespace('jmvcore')) R6::R6Class(
    "scat2Class",
    inherit = scat2Base,
    private = list(
        .init = function() {

            image <- self$results$scat2
            size <- private$.plotSize()
            image$setSize(size$width, size$height)

            image2 <- self$results$scat3
            # size <- private$.plotSize()
            # image2$setSize(size$width, size$height)
        ...
        .run = function() {
            ...
            groupCol <- self$data[[group]]
            depCol <- self$data[[dep]]
            mydata2 <- data.frame(mygroup=groupCol, mydep=depCol)
            mydata2 <- jmvcore::naOmit(mydata2)
            # image2 <- self$results$scat2
            # image2$setState(mydata2)


            # self$results$statsPlot$setState(list(data=self$data,
            #                                      options=self$options))
            # statsPlotPlot <- mydata %>%
            #     ggstatsplot::ggbetweenstats(data = .data,
            #                                 mygroup,
            #                                 mydep
            #     )
            # print(grid::grid.draw(statsPlotPlot));
            # TRUE;
        ...
        .scat2 = function(image, ggtheme, theme, ...) {
