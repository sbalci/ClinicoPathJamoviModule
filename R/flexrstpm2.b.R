    ...
    private = list(
      ...
      .validateInputs = function() {
        ...
        if (nrow(self$data) < private$MIN_OBSERVATIONS) {
          return(list(
            valid = FALSE,
            message = paste(
              " Insufficient data: Need at least", private$MIN_OBSERVATIONS,
        ...
        tryCatch(
          {
            ...
            if (n_events < 10) {
              return(list(
                valid = FALSE,
                message = paste0(
                  " INSUFFICIENT EVENTS: Only ", n_events, " events detected. ",
      ...
      .generateSummaries = function(model_results, time_varying_results) {
        ...
        summary_text <- paste0(
          ...
          if (!is.null(time_varying_results)) {
            paste0(
              "Time-varying effects were included for ", length(time_varying_results), " covariates ",
              "using ", model_results$tvc_df, " degrees of freedom for spline interactions. "
            )
          } else {
            "All covariate effects were assumed to be time-constant. "
          },
      ...
      .parsePredictionTimes = function(times_string) {
        if (is.null(times_string) || nchar(trimws(times_string)) == 0) {
          return(c(1, 2, 5, 10))
        }
                ...
                if (abs(estimate) < 0.01) {
                  ...
                  if (p_val < 0.05) {
                    interpretation <- sprintf(
                      paste0(
                        "%s a %s increased risk: %.1f%% higher hazard (HR=%.2f, %s). ",
                  ...
                  if (p_val < 0.05) {
                    interpretation <- sprintf(
                      paste0(
                        "%s a %s protective effect: %.1f%% lower hazard (HR=%.2f, %s). ",
        ...
        tryCatch(
          {
            ...
            p <- ggplot(plot_data, aes(x = time, y = hazard)) +
              ...
              labs(
                title = "Hazard Function Over Time",
                x = "Time",
                y = "Hazard Rate"
              ) +
              theme_minimal() +
        ...
        tryCatch(
          {
            ...
            p <- ggplot(plot_data, aes(x = time, y = basis_value, color = basis_function)) +
              geom_line(size = 1) +
              labs(
                title = paste("Spline Basis Functions (df =", df, ")"),
                x = "Time",
        ...
        tryCatch(
          {
            ...
            p <- ggplot(plot_data, aes(x = time, y = derivative_value, color = derivative_type)) +
              ...
              labs(
                title = "Hazard Function Derivatives",
                x = "Time",
                y = "Derivative Value"
              ) +
              theme_minimal() +
