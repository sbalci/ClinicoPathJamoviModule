...
ihcdiagnosticClass <- if (requireNamespace("jmvcore", quietly = TRUE)) {
    R6::R6Class(
        "ihcdiagnosticClass",
        ...
        private = list(
                ...
                if (length(levels(groups)) == 2) {
                    ...
                    for (i in seq_along(levels(groups))) {
                        ...
                        for (marker in markers) {
                            ...
                            if (self$options$crossValidation) {
                                perf <- private$.calculateBinaryPerformanceCV(
                                    marker_data, binary_outcome,
                                    paste(marker, "vs", target_group)
                                )
                            } else {
                                perf <- private$.calculateBinaryPerformance(
                                    marker_data, binary_outcome,
                                    paste(marker, "vs", target_group)
                                )
                            }
            ...
            .plotDiagnosticPerformance = function(image, ...) {
                if (is.null(private$.performance_results) || length(private$.performance_results) == 0) {
                    plot(1, 1,
                        type = "n", main = "No performance data available",
                    ...
                    <ul>
                        ...
                        <ul>
                            ...
                            <li>Clinical utility: %s</li>
                                ...
                                } else if (diff_accuracy > 70) {
                                    "Promising, needs optimization"
                                } else {
                                    "Requires additional markers or refinement"
                                }
                            ))
                        }
