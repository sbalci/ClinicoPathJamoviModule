    ...
    private = list(
        ...
        .selectModel = function(processed_results) {
            ...
            if (method == "highest_posterior") {
                ...
                window_models <- Filter(
                    function(m) m$probability >= threshold_prob,
        ...
        .populateInclusionProbsTable = function(results) {
            ...
            for (i in seq_along(var_names)) {
                ...
                evidence_strength <- if (log_bf > 3) "Very Strong" else if (log_bf > 1) "Strong" else if (log_bf > 0.5) "Moderate" else if (log_bf > 0) "Weak" else "Against"
