    ...
    R6::R6Class(
        ...
        private = list(
                ...
                tryCatch(
                    {
                        ...
                        metrics <- list(
                            list(
                                metric = "C-index", value = concordance, interpretation =
                                    if (concordance > 0.7) {
                                        "Good discrimination"
                            ...
                            list(
                                metric = "Log-rank p-value", value = logrank_p, interpretation =
                                    if (logrank_p < 0.001) {
                                        "Highly significant"
            ...
            .performLandmarkAnalysis = function() {
                ...
                landmark_surv <- survival::Surv(
                    time_data[landmark_idx] - landmark_time,
            ...
            .interpretHazardRatio = function(hr, p_value, marker_name) {
                if (is.na(hr)) {
                    return(list(interpretation = "Not estimable", clinical = ""))
                }

                direction <- if (hr > 1) "increased" else "decreased"
                magnitude <- if (hr > 2 || hr < 0.5) {
                    "substantially"
