    ...
    private = list(
        ...
        .run = function() {
            ...
            if (isTRUE(opt$show_r_code)) {
                # Safe R-string literal: deparse() emits a properly quoted/escaped
                # R literal so embedded ", \, or newlines cannot break out.
                .lit <- function(x) paste(
                    deparse(x, width.cutoff = 500L),
                    collapse = ""
                )

                # Bare-identifier positions: composeTerm() backtick-quotes any
                # name that isn't a syntactic identifier, blocking injection
                # via column names like `` x); system("…"); Surv(t,e ``.
                dep_time_t  <- jmvcore::composeTerm(opt$dep_time)
                dep_event_t <- jmvcore::composeTerm(opt$dep_event)
                adj_terms   <- if (!is.null(opt$adjusters))
                    vapply(opt$adjusters, jmvcore::composeTerm, character(1))
                else character(0)
                cox_rhs <- paste(c("biomarker_factor", adj_terms),
                                 collapse = " + ")

                model_lit       <- .lit(private$.buildModelSyntax(opt$indicators))
                estimator_lit   <- .lit(as.character(est_spec$estimator))
                score_lit       <- .lit(as.character(opt$factor_score_method))
                event_level_lit <- .lit(as.character(opt$event_level))

                ordered_arg <- if (length(est_spec$ordered) > 0L)
                    paste0(", ordered = ", .lit(as.character(est_spec$ordered)))
                    else ""
                missing_arg <- if (!is.null(est_spec$missing))
                    paste0(", missing = ", .lit(as.character(est_spec$missing)))
                    else ""
...
fit <- lavaan::cfa(
    ...
    data  = your_data,
    std.lv = TRUE,
    estimator = ", estimator_lit, ordered_arg, missing_arg, "
)
...
cox_fit <- survival::coxph(
    survival::Surv(", dep_time_t, ", ", dep_event_t,
                  " == ", event_level_lit, ") ~ ",
                  cox_rhs, ",
    data = your_data
)
