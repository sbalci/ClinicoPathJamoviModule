...
ihcscoringClass <- if (requireNamespace("jmvcore", quietly = TRUE)) {
    R6::R6Class(
        "ihcscoringClass",
        ...
        private = list(
                ...
                if (self$options$multiple_cutoffs) {
                    tryCatch(
                        ...
                        error = function(e) {
                            ...
                            self$results$interpretation$setContent(
                                paste0(
                                    self$results$interpretation$content,
            ...
            .agreementplot = function(image, ggtheme, theme, ...) {
                ...
                p <- ggplot2::ggplot(data, ggplot2::aes(x = Mean, y = Difference)) +
                    ...
                    ggplot2::geom_hline(
                        yintercept = unique(data$MeanDiff) + 1.96 * unique(data$SD),
                ...
                interpretation <- paste0(
                    ...
                    ifelse(abs(correlation) >= 0.90, "excellent agreement",
                        ifelse(abs(correlation) >= 0.75, "good agreement",
                            ifelse(abs(correlation) >= 0.50, "moderate agreement", "poor agreement")
                        )
                    ), ")</p>",
...
")
            ...
            .performMultipleCutoffAnalysis = function(intensity, proportion, groups) {
                ...
                if (is.null(self$results$multiplecutoffs)) {
                    return()
                }
                ...
                for (i in seq_len(nrow(results_data))) {
                    cutoff_table$addRow(values = list(
                        ...
                        confidence_interval = paste0(
                            round(binom.test(
                                results_data$positive_cases[i],
                                results_data$total_cases[i]
                            )$conf.int[1] * 100, 1),
                            " - ",
                            round(binom.test(
                                results_data$positive_cases[i],
            ...
            .performCPSAnalysis = function(intensity, proportion, groups) {
                ...
                cps_stats_table$addRow(values = list(
                    statistic = "CPS Range",
                    value = paste(
                        round(min(cps_scores, na.rm = TRUE), 1), "-",
            ...
            .generateClinicalSummary = function(intensity, proportion) {
                ...
                summary_html <- paste0(
                    ...
                    if (positive_percent >= 50) {
                        ...
                        "Moderate biomarker expression observed; clinical correlation recommended."
                    } else {
                        "Low biomarker expression in this cohort; negative predictive value for targeted therapies."
                    },
                    "</div>",
                    "</div>"
                )
