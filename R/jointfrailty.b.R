...
#' @importFrom ggplot2 ggplot aes geom_line geom_step geom_point geom_histogram geom_density labs
        ...
        .fitJointFrailtyModel = function(prepared_data) {
            ...
            tryCatch({
                ...
                if (!is.null(rec_covars) && length(rec_covars) > 0) {
                    ...
                    rec_formula_str <- paste("Surv(time, event) ~", paste(jmvcore::composeTerms(as.list(rec_covars)), collapse = " + "))
                ...
                if (!is.null(term_covars) && length(term_covars) > 0) {
                    term_formula_str <- paste("Surv(terminal_time, terminal_event) ~",
                                            paste(jmvcore::composeTerms(as.list(term_covars)), collapse = " + "))
