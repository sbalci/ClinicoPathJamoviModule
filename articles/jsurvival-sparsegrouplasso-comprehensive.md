...
> **Not yet released.** The `sparsegrouplasso` analysis is on a
...
variables within it. This is implemented via `glmnet`’s elastic net Cox
regression with penalty.factor weighting for group structure.

The module supports 5 grouping strategies (factor-based, custom,
pathway-based, variable-type, correlation-based), 4 model selection
criteria (CV deviance, AIC, BIC, EBIC), adaptive weighting from initial
ridge/LASSO/univariate models, repeated CV, bootstrap confidence
intervals, and stability selection.

This analysis is particularly valuable for genomic survival studies
where genes belong to pathways (retain pathway but drop uninformative
genes), clinical prediction models where factors have multiple
categories (drop unimportant levels), and any scenario requiring
structured yet sparse variable selection.
...
Look for: suitability report (EPV, multicollinearity), summary table
(optimal lambda, variables/groups selected), coefficients with hazard
ratios, group structure showing which groups were selected, and the
three default plots.
...
grouped together. This ensures e.g., all `ecog` levels are selected or
dropped as a unit.
...
Look for: group structure table showing `ecog` (3 dummy vars in one
group) and `histology` (2 dummy vars in one group) with descriptive
names.
...
Compare the two: lower alpha tends to select/exclude entire groups,
higher alpha allows individual variable selection within retained
groups.
...
BIC typically selects fewer variables than CV deviance or AIC due to its
...
Format: semicolons separate groups, commas separate variable indices
within groups. Here: {age,smoking_py}, {tumor_size,pdl1}, {crp,nlr},
{albumin,ldh}.
...
Uses initial ridge Cox estimates to weight the penalty - variables with
stronger initial effects get less penalization.
...
Look for: the Adaptive Weights table showing individual and group
...
Look for: CI Lower and CI Upper columns populated in the coefficients
table, and selection frequency showing bootstrap inclusion rates.
...
Look for: stability results table with selection probability, first/last
selected lambda, and the stability plot with threshold line.
...
Repeated CV averages error curves across multiple random fold
assignments for more stable lambda selection.
...
With 40 predictors and 100 observations, this is a p \> n/3 scenario
where regularization is essential.
...
Look for: the regularization path table showing how variables enter/exit
across lambda values, and the sparsity pattern plot.
...
- Simon N, Friedman J, Hastie T, Tibshirani R (2013). “A Sparse-Group
  Lasso.” *Journal of Computational and Graphical Statistics*, 22(2),
  231-245.
...
- Therneau TM (2026). *survival: Survival Analysis*. R package version
  3.8-6.
- Meinshausen N, Buhlmann P (2010). “Stability selection.” *JRSS-B*,
  72(4), 417-473.
