...
for(f in test_files) {
  ...
  if(grepl("psychopdaROC", f)) {
    start_idx <- grep("^psychopdaROC <- function", content)[1]
    if(!is.na(start_idx)) {
      end_idx <- grep("^\\}", content)
    ...
    start_idx <- grep("^enhancedROC <- function", content)[1]
    if(!is.na(start_idx)) {
      end_idx <- grep("^\\}", content)
      end_idx <- end_idx[end_idx > start_idx][1]
      new_wrapper <- c(
    ...
    start_idx <- grep("^agreement <- function", content)[1]
    if(!is.na(start_idx)) {
      end_idx <- grep("^\\}", content)
      end_idx <- end_idx[end_idx > start_idx][1]
      new_wrapper <- c(
