...
cat(
  "Hemoglobin range:", round(min(hemoglobin_data$hemoglobin_g_dl, na.rm = TRUE), 1), "-",
...
cat(
  "IL-6 range:", round(min(biomarker_data$il6_pg_ml), 2), "-",
...
cat(
  "QOL score range:", round(min(quality_life_data$qol_score), 1), "-",
...
cat(
  "Tumor size range:", round(min(tumor_response_data$tumor_size_cm), 1), "-",
