...
cancer_screening_data$biomarker_panel_prob <- with(
  cancer_screening_data,
  plogis(-3 + 2.5 * cancer_detected + rnorm(500, 0, 0.3))
)
cancer_screening_data$single_biomarker_prob <- with(
  cancer_screening_data,
  plogis(-3.2 + 2 * cancer_detected + rnorm(500, 0, 0.4))
)
cancer_screening_data$clinical_score_prob <- with(
  cancer_screening_data,
...
treatment_decision_data$genomic_risk_score <- with(
  treatment_decision_data,
  plogis(-1.2 + 2.8 * treatment_failure + rnorm(300, 0, 0.4))
)
treatment_decision_data$clinical_prediction_model <- with(
  treatment_decision_data,
  plogis(-1.0 + 2.3 * treatment_failure + rnorm(300, 0, 0.5))
)
treatment_decision_data$combined_model <- with(
  treatment_decision_data,
...
diagnostic_test_data$rapid_test_prob <- with(
  diagnostic_test_data,
  plogis(-2.0 + 3.5 * disease_present + rnorm(400, 0, 0.6))
)
diagnostic_test_data$standard_test_prob <- with(
  diagnostic_test_data,
  plogis(-1.2 + 4.0 * disease_present + rnorm(400, 0, 0.4))
)
diagnostic_test_data$novel_biomarker_prob <- with(
  diagnostic_test_data,
...
external_prevalence_data$model_score_A <- with(
  external_prevalence_data,
  plogis(-0.5 + 2.2 * outcome + rnorm(150, 0, 0.5))
)
external_prevalence_data$model_score_B <- with(
  external_prevalence_data,
