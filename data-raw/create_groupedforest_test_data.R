...
create_groupedforest_multi_subgroups <- function() {
  ...
  data.frame(
    ...
    molecular_subtype = factor(sample(c("Type A", "Type B", "Type C", "Type D"), n,
      replace = TRUE,
      prob = c(0.3, 0.25, 0.25, 0.2)
    )),
    risk_category = factor(sample(c("Low Risk", "Intermediate Risk", "High Risk"), n,
      replace = TRUE,
      prob = c(0.4, 0.35, 0.25)
    )),
    histologic_grade = factor(sample(c("Grade 1", "Grade 2", "Grade 3"), n,
      replace = TRUE,
...
create_groupedforest_biomarker_data <- function() {
  ...
  data.frame(
    ...
    biomarker_level = factor(sample(c("High", "Intermediate", "Low"), n,
      replace = TRUE,
      prob = c(0.3, 0.4, 0.3)
    )),
...
create_groupedforest_interaction_data <- function() {
  ...
  data.frame(
    ...
    genetic_profile = factor(sample(c("Profile A", "Profile B", "Profile C"), n,
      replace = TRUE,
      prob = c(0.45, 0.35, 0.20)
    )),
    disease_severity = factor(sample(c("Mild", "Moderate", "Severe"), n,
      replace = TRUE,
      prob = c(0.25, 0.50, 0.25)
    )),
    comorbidity_burden = factor(sample(c("None", "Low", "High"), n,
      replace = TRUE,
      prob = c(0.3, 0.5, 0.2)
    )),
    ...
    ethnicity = factor(sample(c("Caucasian", "Asian", "Hispanic", "Other"), n,
      replace = TRUE,
