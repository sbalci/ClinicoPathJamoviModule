...
All data files are available in `data/` folder as `.rda`, `.csv`,
`.xlsx`, and `.omv`. Open the `.omv` (or `.csv`) in jamovi.
...
**Options covered:** `dependentVars`, `classVar`, `positiveClass`,
`direction` (\>=, \<=), `method` (maximize_metric), `metric` (youden),
`sensSpecTable`, `showThresholdTable`, `maxThresholds`
...
| 5 | `psychopdaROC_test` | disease_status / “Disease” | biomarker | `metric`: abs_d_sens_spec (minimize |
...
**Options covered:** All 12 `method` values (maximize_metric,
minimize_metric, maximize_loess_metric, minimize_loess_metric,
maximize_boot_metric, minimize_boot_metric, oc_youden_kernel,
oc_youden_normal, oc_manual, oc_cost_ratio, oc_equal_sens_spec,
oc_closest_01), all 16 `metric` values, `specifyCutScore`,
`costratioFP`, `boot_runs`, `seed`
...
**Options covered:** `tol_metric`, `break_ties` (mean/median/c),
`allObserved`
...
**Options covered:** multiple `dependentVars`, `combinePlots`,
`delongTest`
...
**Options covered:** `plotROC`, `showOptimalPoint`, `displaySE`,
`smoothing`, `showConfidenceBands`, `cleanPlot`, `legendPosition`
(none/right/bottom/topleft/topright), `directLabel`, `interactiveROC`
...
**Options covered:** `showCriterionPlot`, `showPrevalencePlot`,
`showDotPlot`, `precisionRecallCurve`
...
**Options covered:** `partialAUC`, `partialAUCfrom`, `partialAUCto`,
`rocSmoothingMethod` (none/binormal/density/fitdistr), `bootstrapCI`,
`bootstrapReps`, `quantileCIs`, `quantiles`
...
**Options covered:** `fixedSensSpecAnalysis`, `fixedAnalysisType`
(sensitivity/specificity), `fixedSensitivityValue`,
`fixedSpecificityValue`, `fixedInterpolation` (linear/nearest/stepwise),
`showFixedROC`, `showFixedExplanation`
...
**Options covered:** `calculateIDI`, `calculateNRI`, `refVar`,
`nriThresholds`, `idiNriBootRuns`, `compareClassifiers`
...
**Options covered:** `effectSizeAnalysis`, `powerAnalysis`,
`powerAnalysisType` (post_hoc/prospective/sample_size),
`expectedAUCDifference`, `targetPower`, `significanceLevel`,
`correlationROCs`
...
**Options covered:** `clinicalUtilityAnalysis`, `treatmentThreshold`,
`harmBenefitRatio`, `interventionCost`
...
**Options covered:** `metaAnalysis`, `metaAnalysisMethod`
(fixed/random/both), `heterogeneityTest`, `forestPlot`,
...
**Options covered:** `clinicalMode` (basic/advanced/comprehensive),
`clinicalPreset` (none/screening/confirmation/balanced/research)
...
**Options covered:** AUC=1.0, AUC=0.5, small n, extreme imbalance,
constant predictor, missing data, rare disease, large n
...
**Options covered:** longitudinal marker comparison, disease spectrum
scenarios
...
`dependentVars` — all tests

`classVar` — all tests

`positiveClass` — all tests

`subGroup` — \#35, \#36, \#37, \#79

`direction` (\>=, \<=) — \#1, \#3

### Clinical Mode & Presets

`clinicalMode` (basic/advanced/comprehensive) — \#63, \#64, \#65

`clinicalPreset` (none/screening/confirmation/balanced/research) — \#66,
\#67, \#68

### Cutpoint Optimization

`method` — all 12 methods: \#4-#16
...
`metric` — all 16 metrics: \#4, \#5, \#6

- youden, sum_sens_spec, accuracy, sum_ppv_npv, prod_sens_spec,
  prod_ppv_npv, cohens_kappa, F1_score
- abs_d_sens_spec, abs_d_ppv_npv, odds_ratio, risk_ratio,
  misclassification_cost, total_utility, roc01, p_chisquared
...
`sensSpecTable` (confusion matrix) — \#2

`showThresholdTable`, `maxThresholds` — \#2
...
`showConfidenceBands` — \#23

`legendPosition` (none/right/bottom/topleft/topright) — \#26
...
`showCriterionPlot` — \#29

`showPrevalencePlot` — \#30

`showDotPlot` — \#31

`precisionRecallCurve` — \#32

### Advanced ROC

`partialAUC`, `partialAUCfrom`, `partialAUCto` — \#38, \#39

`rocSmoothingMethod` (none/binormal/density/fitdistr) — \#40

`bootstrapCI`, `bootstrapReps` — \#41

`quantileCIs`, `quantiles` — \#42

### Fixed Sensitivity/Specificity

`fixedSensSpecAnalysis` — \#43

`fixedAnalysisType` (sensitivity/specificity) — \#43, \#44

`fixedSensitivityValue` — \#43

`fixedSpecificityValue` — \#44

`fixedInterpolation` (linear/nearest/stepwise) — \#45

`showFixedROC` — \#46

`showFixedExplanation` — \#46

### Model Comparison

`compareClassifiers` — \#50
...
`effectSizeAnalysis` — \#51

`powerAnalysis` — \#52

`powerAnalysisType` (post_hoc/prospective/sample_size) — \#52, \#53,
\#54

`expectedAUCDifference` — \#53

`targetPower` — \#52

`significanceLevel` — \#52
...
`priorPrecision` — \#55, \#56

### Clinical Utility

`clinicalUtilityAnalysis` — \#57

`treatmentThreshold` — \#57

`harmBenefitRatio` — \#57, \#58
...
`metaAnalysisMethod` (fixed/random/both) — \#59, \#60

`heterogeneityTest` — \#59

`forestPlot` — \#61

`overrideMetaAnalysisWarning` — \#62
